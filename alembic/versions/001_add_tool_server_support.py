...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "display_name", sa.String(length=200), nullable=False
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "bypass_when_unavailable", sa.Boolean(), nullable=False
        ),
        sa.Column("total_calls", sa.Integer(), nullable=False),
        sa.Column("total_errors", sa.Integer(), nullable=False),
        sa.Column(
            "last_called", sa.DateTime(timezone=True), nullable=True
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "conversation_id", sa.UUID(), nullable=True, index=True
        ),
        sa.Column(
            "tool_name",
    ...
    op.create_index(
        "ix_tool_usage_called_at_desc",
        "tool_usage",
        ["called_at"],
        postgresql_using="btree",
        postgresql_ops={"called_at": "DESC"},
    )
    op.create_index(
        "ix_tool_usage_server_tool",
        "tool_usage",
        ["server_id", "tool_id"],
    )
    op.create_index(
        "ix_tool_usage_user_time",
        "tool_usage",
        ["user_id", "called_at"],
    )
