...
class AuditEventEmitter(EventEmitter):
    ...
    async def _log_audit_event(self, event: UnifiedEvent) -> None:
        ...
        error_message = event.data.get("error") or event.data.get(
            "error_message"
        )
    ...
    def _determine_result(self, event: UnifiedEvent) -> AuditResult:
        ...
        if "result" in event.data:
            ...
            return result_mapping.get(
                event.data["result"], AuditResult.SUCCESS
            )
        ...
        if any(
            keyword in event.event_type
            for keyword in ["failed", "error", "unauthorized"]
        ):
...
class AuditEventForwarder:
    ...
    def setup_forwarding(self):
        ...
        event_router.register_emitter(
            EventCategory.AUDIT, self.audit_emitter
        )
    ...
    async def emit_audit_event(
        ...
        **kwargs,
        ...
        priority = priority_mapping.get(
            result.lower(), EventPriority.NORMAL
        )
...
def setup_audit_integration(audit_logger: AuditLogger | None = None):
    ...
    logger.info(
        "Audit logging integration with unified event system is ready"
    )
