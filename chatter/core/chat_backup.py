...
class ChatService:
    """DEPRECATED: Legacy chat service - use RefactoredChatService instead.
    
    This service has been replaced by RefactoredChatService which uses
    a microservice architecture with specialized services:
    ...
    def __init__(self, session: AsyncSession, llm_service: LLMService | None = None) -> None:
        import warnings
        warnings.warn(
            "ChatService is deprecated. Use RefactoredChatService instead.",
            DeprecationWarning,
            stacklevel=2
        )
