...
class MessageImportanceScorer:
    ...
    def __init__(self):
        self.importance_indicators = {
            ...
            "decisions": ["decide", "choose", "select", "pick", "prefer", "recommend"],
            "problems": ["error", "issue", "problem", "bug", "fail", "wrong"],
            "instructions": ["please", "can you", "could you", "would you", "help me"],
            "facts": ["is", "are", "was", "were", "fact", "truth", "actually"],
            
            # Medium importance indicators
            "clarifications": ["clarify", "explain", "meaning", "understand"],
...
class SummaryCache:
    ...
    def _generate_key(self, messages: List[BaseMessage]) -> str:
        ...
        for msg in messages:
            content_hash.update(f"{type(msg).__name__}:{getattr(msg, 'content', '')}".encode())
        return content_hash.hexdigest()
        
    def get_summary(self, messages: List[BaseMessage]) -> Optional[str]:
        ...
        if key in self.cache:
            ...
            if time.time() - entry["timestamp"] < self.ttl_seconds:
                ...
                # Expired entry
                del self.cache[key]
                logger.debug(f"Cache expired for summary key: {key[:8]}...")
                
        return None
        
    def store_summary(self, messages: List[BaseMessage], summary: str) -> None:
        ...
        logger.debug(f"Cached summary for key: {key[:8]}... ({len(messages)} messages)")
    ...
    def clear_expired(self) -> int:
        ...
        if expired_keys:
            logger.debug(f"Cleared {len(expired_keys)} expired summary cache entries")
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "cache_size_bytes": sum(len(str(entry)) for entry in self.cache.values()),
...
class EnhancedMemoryManager:
    ...
    async def manage_memory(
        ...
        if len(messages) <= window_size:
            logger.debug(f"Messages ({len(messages)}) within window size ({window_size}), no management needed")
            return {}
            
        # Split messages
        if self.config.prioritize_recent:
            recent_messages, older_messages = self._split_by_importance(messages, window_size)
        else:
            recent_messages = messages[-window_size:]
            older_messages = messages[:-window_size]
            
        # Create or get cached summary
        summary = await self._create_or_get_summary(older_messages, context, llm)
                ...
                "metadata": {
                    **context.get("metadata", {}),
                    "memory_management": {
                        ...
                        "summary_cached": self._was_summary_cached(older_messages),
                    }
                }
            }
        else:
            # Fallback strategy
            return await self._apply_fallback_strategy(recent_messages, older_messages, context)
            
    def _determine_window_size(self, context: WorkflowNodeContext, messages: List[BaseMessage]) -> int:
        ...
        complexity_score = self._analyze_conversation_complexity(messages, context)
        
        if complexity_score > self.config.complexity_threshold:
            # High complexity - expand window
            expansion_factor = 1 + (complexity_score - self.config.complexity_threshold)
            window_size = int(window_size * expansion_factor)
            logger.debug(f"High complexity ({complexity_score:.2f}), expanding window to {window_size}")
        elif complexity_score < 0.3:
            # Low complexity - can shrink window
            window_size = max(self.config.min_window_size, int(window_size * 0.8))
            logger.debug(f"Low complexity ({complexity_score:.2f}), shrinking window to {window_size}")
            
        # Apply bounds
        window_size = max(self.config.min_window_size, min(self.config.max_window_size, window_size))
        
        return window_size
        
    def _analyze_conversation_complexity(self, messages: List[BaseMessage], context: WorkflowNodeContext) -> float:
        ...
        message_lengths = [len(getattr(msg, "content", "")) for msg in messages[-10:]]
        if message_lengths:
            length_variance = max(message_lengths) - min(message_lengths)
            length_complexity = min(1.0, length_variance / 500.0)  # Normalize
            complexity_factors.append(length_complexity)
            
        # Factor 3: Question frequency (more questions = more complex)
        recent_messages = messages[-10:]
        question_count = sum(1 for msg in recent_messages if "?" in getattr(msg, "content", ""))
        ...
        technical_indicators = ["api", "function", "code", "algorithm", "parameter", "configuration", "implementation"]
        recent_content = " ".join(getattr(msg, "content", "") for msg in recent_messages).lower()
        tech_count = sum(1 for term in technical_indicators if term in recent_content)
        ...
        weights = [0.3, 0.2, 0.2, 0.2, 0.1]  # Tool usage gets highest weight
        complexity_score = sum(factor * weight for factor, weight in zip(complexity_factors, weights))
        
        return complexity_score
        
    def _split_by_importance(self, messages: List[BaseMessage], window_size: int) -> Tuple[List[BaseMessage], List[BaseMessage]]:
        ...
        remaining_messages = messages[:-guaranteed_recent] if guaranteed_recent > 0 else messages
        ...
        logger.debug(f"Split by importance: {len(selected_messages)} kept, {len(older_messages)} to summarize")
    ...
    async def _create_summary(self, messages: List[BaseMessage], llm: BaseChatModel) -> str:
    ...
    async def _create_simple_summary(self, messages: List[BaseMessage], llm: BaseChatModel) -> str:
    ...
    async def _create_intelligent_summary(self, messages: List[BaseMessage], llm: BaseChatModel) -> str:
        ...
        important_messages = [msg for msg, score in scored_messages[:10]]  # Top 10 important
        ...
        if not any(summary.lower().startswith(prefix) for prefix in ["context:", "summary:", "discussion:"]):
            summary = f"Context: {summary}"
            
        return summary
        
    async def _create_structured_summary(self, messages: List[BaseMessage], llm: BaseChatModel) -> str:
    ...
    async def _apply_fallback_strategy(
        ...
        if self.config.fallback_strategy == "truncation":
            logger.info(f"Using truncation fallback: keeping {len(recent_messages)} recent messages")
            return {
                "messages": recent_messages,
                "metadata": {
                    **context.get("metadata", {}),
            ...
            logger.info(f"Using compression fallback: compressed {len(older_messages)} to {len(compressed_older)}")
            return {
                ...
                "metadata": {
                    **context.get("metadata", {}),
                    ...
                    "compressed_messages": len(older_messages) - len(compressed_older),
                }
            }
        elif self.config.fallback_strategy == "skip":
            # Skip memory management this round
            logger.warning("Skipping memory management due to summarization failure")
            return {
                "metadata": {
                    **context.get("metadata", {}),
