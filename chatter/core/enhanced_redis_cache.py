...
class EnhancedRedisCache(CacheInterface):
    ...
    def __init__(
        ...
        logger.debug(
            ...
            redis_url=self.redis_url[:20] + "...",  # Don't log full URL for security
    ...
    async def get(self, key: str) -> Any:
        ...
        """
        if not self.is_valid_key(key) or not await self._ensure_connection():
        ...
        try:
            if self.config.key_prefix:
                ...
                if keys:
                    ...
                    logger.info(f"Redis cache cleared {len(keys)} keys with prefix")
            else:
    ...
    async def keys(self, pattern: Optional[str] = None) -> List[str]:
        ...
        try:
            ...
            if pattern:
                search_pattern = f"{self.config.key_prefix}:*{pattern}*" if self.config.key_prefix else f"*{pattern}*"
            else:
            ...
            if self.config.key_prefix:
                prefix_len = len(self.config.key_prefix) + 1  # +1 for colon
                keys = [key[prefix_len:] for key in keys if key.startswith(f"{self.config.key_prefix}:")]
    ...
    async def mget(self, keys: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            prefixed_keys = [self._add_prefix(key) for key in keys if self.is_valid_key(key)]
            ...
            for i, value in enumerate(values):
                if value is not None:
                    ...
                    try:
                        ...
                        logger.warning(f"JSON decode error for key {original_key}: {e}")
                        self._update_stats(error=True)
                else:
                    self._update_stats(miss=True)
    ...
    async def increment(self, key: str, delta: int = 1) -> int:
        ...
        try:
            ...
            logger.debug("Redis increment", key=key, delta=delta, new_value=result)
            return result
    ...
    async def get_stats(self) -> CacheStats:
        ...
        try:
            ...
            redis_hit_rate = keyspace_hits / total_requests if total_requests > 0 else 0
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            test_key = f"__health_check_{int(asyncio.get_event_loop().time())}"
            test_value = "test"
            ...
            return {
                "status": "healthy" if all([ping_result, set_success, get_success, delete_success]) else "unhealthy",
    ...
    def _handle_connection_error(self, error: Exception) -> None:
        ...
        if any(keyword in error_str for keyword in ["connection", "timeout", "redis"]):
            self._connected = False
            logger.debug("Redis connection marked as failed due to error")