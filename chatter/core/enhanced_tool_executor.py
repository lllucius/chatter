...
class ToolExecutionTracker:
    ...
    def should_continue_execution(self) -> tuple[bool, str]:
        ...
        if total_calls >= self.config.max_total_calls:
            return False, f"Maximum total tool calls ({self.config.max_total_calls}) reached"
            
        # Check per-tool limits
        for tool_name, count in self.tool_call_counts.items():
            max_for_tool = self.config.max_calls_per_tool.get(tool_name, self.config.max_total_calls)
            if count >= max_for_tool:
                return False, f"Maximum calls for tool '{tool_name}' ({max_for_tool}) reached"
        ...
        for tool_name, count in self.consecutive_calls.items():
            if count >= self.config.max_consecutive_calls:
                return False, f"Too many consecutive calls to '{tool_name}' ({count})"
    ...
    def _detect_recursion(self) -> tuple[bool, str]:
        ...
        recent_executions = self.execution_history[-self.config.progress_window:]
    ...
    def _detect_strict_recursion(self, executions: List[Dict[str, Any]]) -> tuple[bool, str]:
        ...
        for execution in executions:
            ...
            if combination in tool_arg_combinations:
                return True, f"Repeated call to {tool_name} with same arguments"
                
            tool_arg_combinations.add(combination)
            
        return False, "No strict recursion detected"
        
    def _detect_adaptive_recursion(self, executions: List[Dict[str, Any]]) -> tuple[bool, str]:
        ...
        if len(set(recent_tools)) == 1 and len(set(recent_results)) == 1:
            return True, f"No progress: {recent_tools[0]} returning same result repeatedly"
            
        # Check for alternating patterns (A -> B -> A -> B)
        if len(executions) >= 4:
            last_4_tools = [e["tool_name"] for e in executions[-4:]]
            if last_4_tools[0] == last_4_tools[2] and last_4_tools[1] == last_4_tools[3]:
                return True, f"Alternating pattern detected: {last_4_tools[0]} <-> {last_4_tools[1]}"
        ...
        if len(recent_errors) >= 2:
            ...
            if len(set(error_tools)) == 1:
                return True, f"Error loop detected for tool: {error_tools[0]}"
                
        return False, "No adaptive recursion detected"
        
    def _detect_lenient_recursion(self, executions: List[Dict[str, Any]]) -> tuple[bool, str]:
        ...
        if (len(set(recent_tools)) == 1 and 
            len(set(recent_results)) <= 2 and  # Allow slight variation
            all(not e["success"] for e in executions[-3:])):  # All recent calls failed
            return True, f"Clear infinite loop: {recent_tools[0]} failing repeatedly"
    ...
    def get_execution_summary(self) -> Dict[str, Any]:
        ...
        total_calls = sum(self.tool_call_counts.values())
        successful_calls = sum(1 for e in self.execution_history if e["success"])
        failed_calls = total_calls - successful_calls
        
        avg_execution_time = 0
        if self.execution_history:
            avg_execution_time = sum(e["execution_time_ms"] for e in self.execution_history) / len(self.execution_history)
...
class EnhancedToolExecutor:
    ...
    async def execute_tools(
        ...
        if not tools or not hasattr(last_message, "tool_calls") or not last_message.tool_calls:
            return {}
        ...
        if not should_continue:
            ...
            return self._create_finalization_response(context, tracker, reason)
        ...
        updated_metadata = {
            **context.get("metadata", {}),
            ...
            "recent_tool_results": [r["result"] for r in execution_results[-3:]],
        }
        
        return {
            "messages": tool_messages,
            "tool_call_count": tracker.get_execution_summary()["total_calls"],
    ...
    async def _execute_single_tool(
        ...
        if not tool_obj:
            ...
            tracker.record_tool_call(tool_name, tool_args, error_msg, 0, False)
            return {
                "message": ToolMessage(content=error_msg, tool_call_id=tool_id),
                "execution_info": {"tool_name": tool_name, "result": error_msg, "success": False}
            }
        ...
        try:
            ...
            result_str = str(result) if result is not None else f"Tool {tool_name} completed"
            
            # Record successful execution
            tracker.record_tool_call(tool_name, tool_args, result_str, execution_time_ms, True)
            
            return {
                "message": ToolMessage(content=result_str, tool_call_id=tool_id),
                "execution_info": {
                    "tool_name": tool_name,
                    "result": result_str,
                    "execution_time_ms": execution_time_ms,
            ...
            tracker.record_tool_call(tool_name, tool_args, error_msg, execution_time_ms, False)
    ...
    async def _invoke_tool(self, tool_obj: Any, tool_args: Dict[str, Any]) -> Any:
        ...
        if hasattr(tool_obj, "ainvoke"):
            ...
            raise RuntimeError(f"Tool is not callable")
    ...
    def _get_or_create_tracker(self, context: WorkflowNodeContext) -> ToolExecutionTracker:
        """Get existing tracker from context or create a new one."""
        existing_tracker = context.get("metadata", {}).get("_tool_tracker")
        if existing_tracker and isinstance(existing_tracker, ToolExecutionTracker):
