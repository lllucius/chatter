...
class LangGraphWorkflowManager:
        ...
        try:
            if (
                ...
                try:
                    ...
                    self.checkpointer = (
                        await self._redis_context_manager.__aenter__()
                    )
    ...
    async def cleanup(self) -> None:
        ...
        if self._redis_context_manager:
            try:
                await self._redis_context_manager.__aexit__(
                    None, None, None
                )
            except Exception as e:
    ...
    def validate_workflow_definition(
        ...
        try:
            ...
            for node in nodes:
                ...
                if "type" not in node:
                    errors.append(
                        f"Node {node['id']} missing required 'type' field"
                    )
                    continue
                ...
                if node_type not in supported_types:
                    ...
                    try:
                        ...
                        errors.append(
                            f"Node {node['id']} configuration error: {str(e)}"
                        )
            ...
            for edge in edges:
                if "source" not in edge or "target" not in edge:
                    errors.append(
                        "Edge missing required 'source' or 'target' field"
                    )
                    continue
