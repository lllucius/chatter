...
class MultiTierCache(CacheInterface):
    ...
    def __init__(
        ...
        if l1_config is None:
            l1_config = CacheConfig(
                ...
                eviction_policy="lru",
                key_prefix=f"{self.config.key_prefix}:l1" if self.config.key_prefix else "l1",
        ...
        l2_config = CacheConfig(
            ...
            key_prefix=f"{self.config.key_prefix}:l2" if self.config.key_prefix else "l2",
            enable_stats=self.config.enable_stats
        )
        ...
        self._promote_threshold = 2  # Promote to L1 after this many L2 hits
    ...
    async def connect(self) -> bool:
        ...
        if not l2_connected:
            logger.warning("L2 (Redis) cache unavailable, operating in L1-only mode")
    ...
    async def set(self, key: str, value: Any, ttl: Optional[int] = None) -> bool:
        ...
        if success:
            ...
            logger.warning("Multi-tier cache set failed on both layers", key=key)
            self._update_stats(error=True)
    ...
    async def get_stats(self) -> CacheStats:
        ...
        combined_stats = CacheStats(
            ...
            cache_hits=self._stats.cache_hits + l1_stats.cache_hits + l2_stats.cache_hits,
            cache_misses=self._stats.cache_misses + l1_stats.cache_misses + l2_stats.cache_misses,
            memory_usage=l1_stats.memory_usage + l2_stats.memory_usage,
            evictions=l1_stats.evictions + l2_stats.evictions,
            errors=self._stats.errors + l1_stats.errors + l2_stats.errors
        )
        
        # Recalculate hit rate
        if combined_stats.total_requests > 0:
            combined_stats.hit_rate = combined_stats.cache_hits / combined_stats.total_requests
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        overall_status = "healthy" if l1_health.get("status") == "healthy" else "degraded"
        
        # If both are unhealthy, mark as unhealthy
        if l1_health.get("status") != "healthy" and l2_health.get("status") != "healthy":
    ...
    async def _consider_promotion(self, key: str, value: Any) -> None:
        ...
        self._promotion_counts[key] = self._promotion_counts.get(key, 0) + 1
