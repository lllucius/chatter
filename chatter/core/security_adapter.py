...
class SecurityEventEmitter(EventEmitter):
        ...
        try:
            ...
            if event.category == EventCategory.SECURITY:
                ...
                if self.monitoring_service and security_event:
                    await self.monitoring_service.record_security_event(
                        security_event
                    )
    ...
    def _convert_to_security_event(
        ...
        severity = severity_mapping.get(
            event.priority, SecurityEventSeverity.MEDIUM
        )
...
class SecurityEventForwarder:
    ...
    def setup_forwarding(self):
        ...
        event_router.register_emitter(
            EventCategory.SECURITY, self.security_emitter
        )

        logger.info(
            "Set up security event forwarding to unified system"
        )
    ...
    async def emit_security_event(
        ...
        **kwargs,
        ...
        priority = priority_mapping.get(
            severity.lower(), EventPriority.NORMAL
        )
...
def setup_security_integration():
    ...
    logger.info(
        "Security monitoring integration with unified event system is ready"
    )
...
async def emit_login_event(
    ...
    **kwargs,
    ...
    """Emit a login event."""
    event_type = (
        "auth.login_success" if success else "auth.login_failure"
    )
...
async def emit_account_event(
    ...
    **kwargs,
    ...
    severity = (
        "high" if action in ["locked", "deactivated"] else "medium"
    )
