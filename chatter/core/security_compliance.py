...
class SecurityComplianceChecker:
    ...
    async def check_compliance(self, standard: SecurityStandard = None) -> Dict[str, Any]:
        ...
        if standards_to_check:
            report["overall_score"] = total_score / len(standards_to_check)
    ...
    async def _check_nist_compliance(self) -> Dict[str, Any]:
        ...
        return self._calculate_compliance_score(checks, "NIST Cybersecurity Framework")
    ...
    async def _check_jwt_compliance(self) -> Dict[str, Any]:
        ...
        return self._calculate_compliance_score(checks, "JWT Best Practices")
    ...
    async def _check_password_compliance(self) -> Dict[str, Any]:
        ...
        return self._calculate_compliance_score(checks, "Password Security")
    ...
    def _check_jwt_secret_strength(self) -> ComplianceLevel:
        ...
        secret_length = len(settings.secret_key) if hasattr(settings, 'secret_key') else 0
        if secret_length >= 32:
            return ComplianceLevel.COMPLIANT
    ...
    def _check_token_expiration(self) -> ComplianceLevel:
        ...
        refresh_expire = getattr(settings, 'refresh_token_expire_days', 7)
    ...
    def _check_secure_transport(self) -> ComplianceLevel:
        ...
        return ComplianceLevel.COMPLIANT if force_https else ComplianceLevel.PARTIALLY_COMPLIANT
    
    def _calculate_compliance_score(self, checks: List[Dict[str, Any]], category: str) -> Dict[str, Any]:
        ...
        final_score = (weighted_score / applicable_weight) if applicable_weight > 0 else 100
        ...
        for check in checks:
            if check["status"] == ComplianceLevel.NON_COMPLIANT:
                critical_issues.append(f"{category}: {check['name']} - {check['description']}")
            elif check["status"] == ComplianceLevel.PARTIALLY_COMPLIANT:
                recommendations.append(f"{category}: Improve {check['name']} - {check['description']}")
    ...
    def _generate_summary(self, report: Dict[str, Any]) -> Dict[str, Any]:
        ...
        summary = {
            "overall_compliance_level": self._get_compliance_level(overall_score),
        ...
        category_scores = []
        for std_name, std_data in report["standards"].items():
            category_scores.append((std_data["category"], std_data["score"]))
        
        category_scores.sort(key=lambda x: x[1], reverse=True)
        summary["top_strengths"] = [cat for cat, score in category_scores[:3] if score >= 80]
        summary["top_weaknesses"] = [cat for cat, score in category_scores[-2:] if score < 70]
...
async def generate_security_report(standards: List[SecurityStandard] = None) -> str:
    ...
    for report in reports:
        ...
        output.append(f"Compliance Level: {report['summary']['overall_compliance_level']}")
        output.append("")
        ...
        if report['recommendations']:
            ...
            if len(report['recommendations']) > 5:
                output.append(f"  ... and {len(report['recommendations']) - 5} more")
            output.append("")
        
        output.append("📊 DETAILED SCORES:")
        for std_name, std_data in report['standards'].items():
            output.append(f"  {std_data['category']}: {std_data['score']:.1f}% ({std_data['level']})")
        output.append("")
