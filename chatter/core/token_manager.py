...
class TokenManager:
    ...
    async def create_tokens(self, user: User) -> dict[str, Any]:
        ...
        # Create tokens
        access_token = create_access_token(access_payload, access_expires)
        refresh_token = create_refresh_token(refresh_payload, refresh_expires)
        
        # Store token metadata for revocation tracking
        await self._store_token_metadata(jti, user.id, access_payload["session_id"])
    ...
    async def revoke_token(self, jti: str, reason: str = "logout") -> bool:
        ...
        try:
            ...
            await self.cache.set(f"blacklist:{jti}", blacklist_data, expire_time)
    ...
    async def revoke_all_user_tokens(self, user_id: str, reason: str = "security") -> int:
        ...
        try:
            ...
            logger.info(f"Revoked {revoked_count} tokens for user", user_id=user_id, reason=reason)
            return revoked_count
            
        except Exception as e:
            logger.error(f"Bulk token revocation failed: {e}", user_id=user_id)
            return 0
    ...
    async def is_token_blacklisted(self, jti: str) -> bool:
        ...
        try:
            if not self.cache:
                ...
                logger.warning("No cache service available for blacklist check")
                return True
    ...
    async def validate_token_security(self, token_payload: dict) -> bool:
        ...
        try:
            ...
            if issued_at:
                token_age = datetime.now(UTC) - datetime.fromtimestamp(issued_at, UTC)
                max_age = timedelta(days=settings.refresh_token_expire_days)
                
                if token_age > max_age:
                    logger.warning(f"Token is too old", jti=jti, age_days=token_age.days)
                    return False
    ...
    async def refresh_access_token(self, refresh_token_payload: dict) -> dict[str, Any]:
        ...
        if not await self.validate_token_security(refresh_token_payload):
        ...
        # Create new tokens
        new_access_token = create_access_token(access_payload, access_expires)
        new_refresh_token = create_refresh_token(refresh_payload, refresh_expires)
    ...
    async def _store_token_metadata(self, jti: str, user_id: str, session_id: str):
        ...
        try:
            ...
            expire_time = timedelta(days=settings.refresh_token_expire_days)
            await self.cache.set(f"token_meta:{jti}", metadata, expire_time)
            
            # Update user's active tokens list
            await self._add_to_user_tokens(user_id, jti)
            
        except Exception as e:
            logger.error(f"Failed to store token metadata: {e}", jti=jti)
    ...
    async def _add_to_user_tokens(self, user_id: str, jti: str):
        ...
        try:
            ...
            # Store updated list
            expire_time = timedelta(days=settings.refresh_token_expire_days)
            await self.cache.set(user_tokens_key, user_tokens, expire_time)
