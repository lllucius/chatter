...
class UnifiedModelRegistryCache:
    ...
    async def get_default_provider(self, model_type: ModelType) -> Optional[str]:
    ...
    async def set_default_provider(self, model_type: ModelType, provider_id: str) -> bool:
    ...
    async def set_default_model(self, model_type: ModelType, model_id: str) -> bool:
    ...
    async def invalidate_defaults(self, model_type: Optional[ModelType] = None) -> bool:
        ...
        if model_type:
            ...
            provider_key = self.cache.make_key("default_provider", model_type.value)
            model_key = self.cache.make_key("default_model", model_type.value)
    ...
    async def set_provider(self, provider_id: str, provider_data: Dict[str, Any]) -> bool:
        """Cache provider data.
        ...
        """
        key = self.cache.make_key("provider", provider_id)
        return await self.cache.set(key, provider_data, self.provider_ttl)
    ...
    async def set_provider_list(
        ...
        return await self.cache.set(key, (providers, total), self.list_ttl)
    ...
    async def warm_frequent_data_cache(self, session) -> None:
        ...
        try:
            ...
            for provider in providers:
                ...
                await self.cache.set_provider(provider.id, provider_dict)
...
_registry_cache: Optional[UnifiedModelRegistryCache] = None
