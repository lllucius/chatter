...
class UnifiedWorkflowCache:
    ...
    def __init__(self, cache: Optional[CacheInterface] = None, max_size: int = 100):
        ...
        logger.debug("Unified workflow cache initialized", max_size=max_size)
...
class UnifiedLazyToolLoader:
    ...
    async def get_tools(self, required_tools: Optional[list[str]] = None) -> list[Any]:
        ...
        tool_keys = [self.cache.make_key("tool", tool_name) for tool_name in required_tools]
    ...
    async def _load_specific_tool(self, tool_name: str) -> Optional[Any]:
    ...
    async def get_cached_tool_names(self) -> list[str]:
        ...
        for key in tool_keys:
            ...
            if ":" in key:
                tool_name = key.split(":")[-1]  # Get last part after colon
...
_workflow_cache: Optional[UnifiedWorkflowCache] = None
_lazy_tool_loader: Optional[UnifiedLazyToolLoader] = None
