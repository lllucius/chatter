...
class WorkflowNodeExecutor:
    ...
    async def execute_node(
        ...
        if node_type == 'start':
            return await self._execute_start_node(node, context, conversation, spec)
        elif node_type == 'model' or node_type == 'llm':
            return await self._execute_llm_node(node, context, conversation, spec)
        elif node_type == 'retrieval':
            return await self._execute_retrieval_node(node, context, conversation, spec)
        elif node_type == 'tool':
            return await self._execute_tool_node(node, context, conversation, spec)
        elif node_type == 'end':
            return await self._execute_end_node(node, context, conversation, spec)
        else:
    ...
    async def _execute_start_node(
        ...
        conversation: Conversation, spec: WorkflowSpec
    ) -> dict[str, Any]:
    ...
    async def _execute_llm_node(
        ...
        system_prompt = node_config.get('system_prompt', spec.system_prompt)
        ...
        if context.get('retrieval_context'):
            context_prompt = f"Context: {context['retrieval_context']}\n\n"
    ...
    async def _execute_retrieval_node(
        ...
        if not spec.capabilities.enable_retrieval:
            logger.warning("Retrieval node in workflow but retrieval not enabled")
            return context
        ...
        try:
            ...
            context['retrieval_context'] = f"Retrieved context for: {user_message[:100]}..."
    ...
    async def _execute_tool_node(
        ...
        if not spec.capabilities.enable_tools:
            logger.warning("Tool node in workflow but tools not enabled")
            return context
    ...
    async def _execute_end_node(
        ...
        if 'start_time' in context:
            context['execution_time_ms'] = int((context['end_time'] - context['start_time']) * 1000)
...
class UnifiedWorkflowEngine:
    ...
    def __init__(self, llm_service, message_service, template_manager=None):
        self.llm_service = llm_service
        self.message_service = message_service
        self.template_manager = template_manager
        self.node_executor = WorkflowNodeExecutor(llm_service, message_service)
    ...
    async def execute_workflow(
        ...
        try:
            ...
            context = {
                ...
                'spec': spec,
                'user_message': input_data.get('message', '') if input_data else ''
            }
            
            if spec.nodes and spec.edges:
                # Execute defined workflow with nodes and edges
                result_context = await self._execute_defined_workflow(spec, conversation, context)
            else:
                # Execute capability-based workflow
                result_context = await self._execute_capability_workflow(spec, conversation, context)
            ...
            if 'llm_response' in result_context:
                ...
                message = Message(
                    ...
                    extra_metadata={
                        ...
                        'execution_context': {
                            k: v for k, v in result_context.items() 
            ...
            usage_info['nodes_executed'] = result_context.get('nodes_executed', 0)
    ...
    async def _execute_defined_workflow(
        ...
        execution_order = self._build_execution_order(spec.nodes, spec.edges or [])
        ...
        for node_id in execution_order:
            ...
            context = await self.node_executor.execute_node(node, context, conversation, spec)
            nodes_executed += 1
    ...
    async def _execute_capability_workflow(
        ...
        if spec.capabilities.enable_retrieval:
            context = await self._simulate_retrieval_node(context, conversation, spec)
            nodes_executed += 1
        
        # LLM node (always present)
        context = await self._simulate_llm_node(context, conversation, spec)
        nodes_executed += 1
        
        # Tool node (if enabled and tools needed)
        if spec.capabilities.enable_tools:
            context = await self._simulate_tool_node(context, conversation, spec)
            nodes_executed += 1
    ...
    async def _simulate_start_node(self, context: dict, spec: WorkflowSpec) -> dict:
        """Simulate start node execution."""
        mock_start_node = {'type': 'start', 'data': {'config': {}}}
        return await self.node_executor.execute_node(mock_start_node, context, None, spec)
    
    async def _simulate_retrieval_node(self, context: dict, conversation: Conversation, spec: WorkflowSpec) -> dict:
        ...
        return await self.node_executor.execute_node(mock_retrieval_node, context, conversation, spec)
    
    async def _simulate_llm_node(self, context: dict, conversation: Conversation, spec: WorkflowSpec) -> dict:
        ...
        return await self.node_executor.execute_node(mock_llm_node, context, conversation, spec)
    
    async def _simulate_tool_node(self, context: dict, conversation: Conversation, spec: WorkflowSpec) -> dict:
        ...
        return await self.node_executor.execute_node(mock_tool_node, context, conversation, spec)
    
    async def _simulate_end_node(self, context: dict, spec: WorkflowSpec) -> dict:
        """Simulate end node execution."""
        mock_end_node = {'type': 'end', 'data': {'config': {}}}
        return await self.node_executor.execute_node(mock_end_node, context, None, spec)
