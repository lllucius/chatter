...
class UnifiedWorkflowExecutor:
    ...
    async def execute(
        ...
        # Determine workflow mode from capabilities
        if chat_request.enable_retrieval and chat_request.enable_tools:
            workflow_mode = "full"
        elif chat_request.enable_tools:
            workflow_mode = "tools"
        elif chat_request.enable_retrieval:
            workflow_mode = "rag"
        else:
            workflow_mode = "plain"
        ...
        try:
            ...
            raise WorkflowExecutionError(
                f"{workflow_mode.title()} workflow execution failed: {str(e)}"
            ) from e
        finally:
    ...
    async def execute_streaming(
        ...
        try:
            ...
            raise WorkflowExecutionError(
                f"{workflow_mode.title()} workflow streaming failed: {str(e)}"
            ) from e
        finally:
            # Clean up resource tracking
            if user_id:
    ...
    async def execute_streaming_with_tracing(
        ...
        try:
            ...
            raise WorkflowExecutionError(
                f"{workflow_mode.title()} workflow streaming with tracing failed: {str(e)}"
            ) from e
        finally:
            # Clean up resource tracking
            if user_id:
    ...
    async def _get_workflow_config(
        ...
        if workflow_mode == "plain":
            ...
            logger.warning(
                f"Unknown workflow type '{workflow_mode}', using plain config"
            )
            config.update(
                {
                    "memory_window": 20,
    ...
    async def _setup_execution(
        ...
        workflow_id = f"{correlation_id}_{workflow_mode}"
    ...
    async def _finalize_streaming_message(
        ...
        if usage_metadata or model_used or response_time_ms:
            ...
            if usage_metadata:
                message.prompt_tokens = usage_metadata.get(
                    "input_tokens"
                )
                message.completion_tokens = usage_metadata.get(
                    "output_tokens"
                )
                message.total_tokens = usage_metadata.get(
                    "total_tokens"
                )
        ...
        if "messages" in result:
            ...
            if messages:
                ...
                if (
                    ...
                    usage_info.update(
                        {
                            "input_tokens": last_message.usage_metadata.get(
                                "input_tokens", 0
                            ),
                            "output_tokens": last_message.usage_metadata.get(
                                "output_tokens", 0
                            ),
                            "total_tokens": last_message.usage_metadata.get(
                                "total_tokens", 0
                            ),
                        }
                    )
