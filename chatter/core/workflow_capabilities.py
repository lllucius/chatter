...
class WorkflowCapabilities:
    ...
    @classmethod
    def from_legacy_type(cls, workflow_type: str) -> WorkflowCapabilities:
        """Create capabilities from legacy workflow type for migration."""
        if workflow_type == "plain":
            return cls()
        elif workflow_type == "rag":
            return cls(enable_retrieval=True, max_documents=10, memory_window=30)
        elif workflow_type == "tools":
            return cls(enable_tools=True, max_tool_calls=10, memory_window=100)
...
class WorkflowSpec:
    ...
    def from_chat_request(cls, chat_request) -> WorkflowSpec:
        ...
        workflow_type = getattr(chat_request, 'workflow_type', None) or getattr(chat_request, 'workflow', 'plain')
    ...
    def from_workflow_definition(cls, definition) -> WorkflowSpec:
        ...
        capabilities = cls._analyze_workflow_capabilities(definition.nodes)
    ...
    def from_chat_workflow_config(cls, config) -> WorkflowSpec:
        ...
        if config.llm_config:
            spec.provider = getattr(config.llm_config, 'provider', spec.provider)
            spec.model = getattr(config.llm_config, 'model', spec.model)
            spec.temperature = getattr(config.llm_config, 'temperature', spec.temperature)
            spec.max_tokens = getattr(config.llm_config, 'max_tokens', spec.max_tokens)
        
        return spec
    
    @staticmethod
    def _analyze_workflow_capabilities(nodes: list[dict[str, Any]] | None) -> WorkflowCapabilities:
