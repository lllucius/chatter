...
class WorkflowLimitManager:
    ...
    def check_concurrent_limit(
        self, user_id: str, limits: WorkflowLimits
    ) -> None:
    ...
    def end_workflow_tracking(
        ...
        if usage is None:
            logger.warning(
                "Attempted to end tracking for unknown workflow",
                workflow_id=workflow_id,
            )
            return WorkflowResourceUsage(start_time=time.time())

        self.user_workflow_counts[user_id] = max(
            0, self.user_workflow_counts.get(user_id, 1) - 1
        )
    ...
    def check_workflow_limits(
        self, workflow_id: str, limits: WorkflowLimits
    ) -> None:
        ...
        if usage.memory_used_mb > limits.max_memory_mb:
            raise WorkflowResourceLimitError(
                "Workflow exceeded memory limit",
                "max_memory",
    ...
    @asynccontextmanager
    async def step_timeout_context(
        self, workflow_id: str, limits: WorkflowLimits
    ):
