...
class WorkflowCache:
    ...
    def _evict_lru(self) -> None:
        ...
        logger.debug(
            "Evicted LRU workflow from cache", cache_key=lru_key
        )
...
class LazyToolLoader:
    ...
    async def get_tools(
        ...
        for tool_name in required_tools:
            ...
            if tool:
                ...
                logger.warning(
                    f"Tool '{tool_name}' not found", tool_name=tool_name
                )
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        avg_load_time = (
            total_load_time / len(self.load_times)
            if self.load_times
            else 0
        )
        ...
        return {
            ...
            "total_tools": (
                len(self._all_tools) if self._all_tools else 0
            ),
