...
class ValidationResult:
    ...
    def __init__(
        self, valid: bool = True, errors: list[str] | None = None
    ):
        self.valid = valid
...
class InputSanitizer:
    ...
    def sanitize_html(html: str) -> str:
        ...
        for tag in dangerous_tags:
            ...
            html = re.sub(
                f'<{tag}[^>]*/?>', '', html, flags=re.IGNORECASE
            )

        return html

    @staticmethod
    def sanitize_query_parameters(
        params: dict[str, str],
    ) -> dict[str, str]:
    ...
    def sanitize_user_input(
        ...
        for key, value in user_input.items():
            if isinstance(value, str):
                ...
                sanitized_value = InputSanitizer.sanitize_text(
                    sanitized_value
                )
                sanitized[key] = sanitized_value
            elif isinstance(value, dict):
                # Recursively sanitize nested dictionaries
                sanitized[key] = InputSanitizer.sanitize_user_input(
                    value
                )
            else:
                sanitized[key] = value
        ...
        for pattern in sql_patterns:
            if re.search(pattern, text, re.IGNORECASE):
                return ValidationResult(
                    valid=False,
                    errors=[
                        "Text contains potentially malicious patterns"
                    ],
                )
...
class WorkflowValidator:
        ...
        **kwargs: Any,
        ...
        for param in required_params:
            if param == "retriever" and retriever is None:
                issues.append(
                    f"Retriever is required for {workflow_type} workflow"
                )
            elif param == "tools" and (not tools or len(tools) == 0):
                issues.append(
                    f"Tools are required for {workflow_type} workflow"
                )
        ...
        cls._validate_workflow_combinations(
            wf_type, retriever, tools, kwargs, issues
        )
    ...
    def _validate_parameter(
        cls, name: str, value: Any, issues: list[str]
    ) -> None:
        ...
        if isinstance(value, str):
            ...
            if max_length and len(value) > max_length:
                issues.append(
                    f"Parameter '{name}' must be <= {max_length} characters"
                )

    @classmethod
    def _validate_tools(
        cls, tools: list[Any], issues: list[str]
    ) -> None:
        """Validate tools list."""
    ...
    def _validate_retriever(
        cls, retriever: Any, issues: list[str]
    ) -> None:
        ...
        for method in required_methods:
            if not hasattr(retriever, method):
                issues.append(
                    f"Retriever missing required method '{method}'"
                )
    ...
    def _validate_workflow_combinations(
        ...
        if (
            workflow_type in [WorkflowType.RAG, WorkflowType.FULL]
            and retriever
        ):
            max_docs = params.get("max_documents", 5)
            if max_docs > 20:
                issues.append(
                    "max_documents should not exceed 20 for performance reasons"
                )

        # Tools-specific validations
        if (
            workflow_type in [WorkflowType.TOOLS, WorkflowType.FULL]
            and tools
        ):
            max_calls = params.get("max_tool_calls", 10)
            if len(tools) > 10 and max_calls > 5:
                issues.append(
                    "When using many tools, consider reducing max_tool_calls"
                )
        ...
        if params.get("enable_memory", False):
            ...
            if memory_window > 50:
                issues.append(
                    "Large memory windows may impact performance"
                )

    @classmethod
    def validate_workflow_parameters(
        cls, workflow_type: str, **params: Any
    ) -> dict[str, Any]:
    ...
    def validate_workflow_permissions(
        cls, workflow_config: dict, user_permissions: dict
    ) -> ValidationResult:
        ...
        if required_role:
            ...
            if user_role != required_role:
                errors.append(
                    f"User role '{user_role}' does not match required role '{required_role}'"
                )
        ...
        for tool in allowed_tools:
            if tool not in user_tools:
                errors.append(
                    f"User does not have access to required tool: {tool}"
                )

        # Check restricted actions
        restricted_actions = workflow_perms.get(
            'restricted_actions', []
        )
    ...
    def validate_workflow_dependencies(
        cls, workflow_config: dict, available_services: list[str]
    ) -> ValidationResult:
        ...
        for service in required_services:
            if service not in available_services:
                errors.append(
                    f"Required service not available: {service}"
                )

        # Check optional services (warnings, not errors)
        optional_services = dependencies.get('optional_services', [])
        missing_optional = [
            s for s in optional_services if s not in available_services
        ]
        if missing_optional:
...
def validate_chat_request(
    ...
    if (
        workflow_type
        not in WorkflowValidator.get_supported_workflow_types()
    ):
        issues.append(f"Invalid workflow type '{workflow_type}'")

    if issues:
        raise WorkflowValidationError(
            "Chat request validation failed", validation_issues=issues
        )
    ...
    if conversation_id is not None:
        if not isinstance(conversation_id, str):
            raise WorkflowValidationError(
                ...
                validation_issues=[
                    "conversation_id must be string type"
                ],
            )
...
class ParameterValidator:
    ...
    def validate_max_tokens(self, max_tokens: int) -> ValidationResult:
        ...
        if not isinstance(max_tokens, int):
            return ValidationResult(
                valid=False, errors=["max_tokens must be an integer"]
            )
    ...
    def validate_model_name(self, model_name: str) -> ValidationResult:
        ...
        if not isinstance(model_name, str):
            return ValidationResult(
                valid=False, errors=["Model name must be a string"]
            )

        if not model_name.strip():
            return ValidationResult(
                valid=False, errors=["Model name cannot be empty"]
            )
        ...
        if not any(
            pattern in model_lower for pattern in valid_patterns
        ):
            return ValidationResult(
                valid=False,
...
class StepValidator:
    ...
    def validate_llm_call_step(self, step: dict) -> ValidationResult:
        ...
        if 'temperature' in config:
            temp_result = self.param_validator.validate_temperature(
                config['temperature']
            )
        ...
        if 'max_tokens' in config:
            tokens_result = self.param_validator.validate_max_tokens(
                config['max_tokens']
            )
    ...
    def validate_condition_step(self, step: dict) -> ValidationResult:
        ...
        if 'condition' not in config:
            errors.append(
                "Condition step requires 'condition' in config"
            )

        if 'true_path' not in config and 'false_path' not in config:
            errors.append(
                "Condition step requires at least one of 'true_path' or 'false_path'"
            )
    ...
    def validate_tool_call_step(self, step: dict) -> ValidationResult:
        ...
        if 'tool_name' not in config:
            errors.append(
                "Tool call step requires 'tool_name' in config"
            )

        if 'parameters' in config and not isinstance(
