...
class ModelDef(Base):
    ...
    model_type: Mapped[str] = mapped_column(String(50), nullable=False)  # llm, embedding
    display_name: Mapped[str] = mapped_column(String(300), nullable=False)
    description: Mapped[Optional[str]] = mapped_column(Text)
    
    # Model configuration
    model_name: Mapped[str] = mapped_column(String(200), nullable=False)  # actual model name for API calls
    max_tokens: Mapped[Optional[int]] = mapped_column(Integer)
    context_length: Mapped[Optional[int]] = mapped_column(Integer)
    dimensions: Mapped[Optional[int]] = mapped_column(Integer)  # for embedding models
    
    # Embedding-specific settings
    chunk_size: Mapped[Optional[int]] = mapped_column(Integer)
    supports_batch: Mapped[bool] = mapped_column(Boolean, default=True)
    max_batch_size: Mapped[Optional[int]] = mapped_column(Integer)
    ...
    embedding_spaces = relationship("EmbeddingSpace", back_populates="model", cascade="all, delete-orphan")
...
class EmbeddingSpace(Base):
    ...
    base_dimensions: Mapped[int] = mapped_column(Integer, nullable=False)  # original model dimensions
    effective_dimensions: Mapped[int] = mapped_column(Integer, nullable=False)  # after reduction
    reduction_strategy: Mapped[str] = mapped_column(String(50), default=ReductionStrategy.NONE)
    
    # Reduction configuration
    reducer_path: Mapped[Optional[str]] = mapped_column(String(500))  # path to joblib reducer file
    reducer_version: Mapped[Optional[str]] = mapped_column(String(100))  # version/hash of reducer
    normalize_vectors: Mapped[bool] = mapped_column(Boolean, default=True)
    
    # Vector store configuration
    distance_metric: Mapped[str] = mapped_column(String(50), default=DistanceMetric.COSINE)
    table_name: Mapped[str] = mapped_column(String(200), nullable=False, unique=True)
    index_type: Mapped[str] = mapped_column(String(50), default="hnsw")  # hnsw, ivfflat
    index_config: Mapped[dict] = mapped_column(JSON, default=dict)  # index-specific parameters
