...
class AgentCreateRequest(BaseModel):
    ...
    personality_traits: list[str] = Field(
        default_factory=list, description="Agent personality traits"
    )
    knowledge_domains: list[str] = Field(
        default_factory=list, description="Knowledge domains"
    )
    response_style: str = Field(
        "professional", description="Response style"
    )
    ...
    primary_llm: str = Field(
        "openai", description="Primary LLM provider"
    )
    fallback_llm: str = Field(
        "anthropic", description="Fallback LLM provider"
    )
    temperature: float = Field(
        0.7, ge=0.0, le=2.0, description="Temperature for responses"
    )
    max_tokens: int = Field(
        4096, ge=1, le=32000, description="Maximum tokens"
    )

    # Performance settings
    max_conversation_length: int = Field(
        50, ge=1, le=1000, description="Maximum conversation length"
    )
    context_window_size: int = Field(
        4000, ge=100, le=32000, description="Context window size"
    )
    response_timeout: int = Field(
        30, ge=1, le=300, description="Response timeout in seconds"
    )

    # Learning settings
    learning_enabled: bool = Field(
        True, description="Enable learning from feedback"
    )
    feedback_weight: float = Field(
        0.1, ge=0.0, le=1.0, description="Weight for feedback learning"
    )
    adaptation_threshold: float = Field(
        0.8, ge=0.0, le=1.0, description="Adaptation threshold"
    )

    # Metadata
    tags: list[str] = Field(
        default_factory=list, description="Agent tags"
    )
...
class AgentUpdateRequest(BaseModel):
    ...
    personality_traits: list[str] | None = Field(
        None, description="Agent personality traits"
    )
    knowledge_domains: list[str] | None = Field(
        None, description="Knowledge domains"
    )
    response_style: str | None = Field(
        None, description="Response style"
    )
    ...
    primary_llm: str | None = Field(
        None, description="Primary LLM provider"
    )
    fallback_llm: str | None = Field(
        None, description="Fallback LLM provider"
    )
    temperature: float | None = Field(
        None, ge=0.0, le=2.0, description="Temperature for responses"
    )
    max_tokens: int | None = Field(
        None, ge=1, le=32000, description="Maximum tokens"
    )

    # Performance settings
    max_conversation_length: int | None = Field(
        None, ge=1, le=1000, description="Maximum conversation length"
    )
    context_window_size: int | None = Field(
        None, ge=100, le=32000, description="Context window size"
    )
    response_timeout: int | None = Field(
        None, ge=1, le=300, description="Response timeout in seconds"
    )

    # Learning settings
    learning_enabled: bool | None = Field(
        None, description="Enable learning from feedback"
    )
    feedback_weight: float | None = Field(
        None, ge=0.0, le=1.0, description="Weight for feedback learning"
    )
    adaptation_threshold: float | None = Field(
        None, ge=0.0, le=1.0, description="Adaptation threshold"
    )
...
class AgentStatsResponse(BaseModel):
    ...
    agent_types: dict[str, int] = Field(
