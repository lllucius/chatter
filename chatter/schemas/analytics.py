...
class ToolServerAnalyticsResponse(BaseModel):
    ...
    server_uptime_stats: dict[str, Any] = Field(
        ..., description="Server uptime statistics"
    )
...
class MetricsSummaryResponse(BaseModel):
    ...
    active_metrics: int = Field(
        ..., description="Number of active metrics"
    )
    data_points_last_hour: int = Field(
        ..., description="Data points collected in last hour"
    )
    storage_size_mb: float = Field(
        ..., description="Storage size in MB"
    )
    oldest_data_point: datetime | None = Field(
        None, description="Oldest data point timestamp"
    )
    latest_data_point: datetime | None = Field(
        None, description="Latest data point timestamp"
    )
...
class CacheStatusResponse(BaseModel):
    ...
    cache_hit_rate: float = Field(..., description="Cache hit rate")
    cached_items_count: int = Field(
        ..., description="Number of cached items"
    )
    cache_size_mb: float = Field(..., description="Cache size in MB")
    last_cleared: datetime | None = Field(
        None, description="Last cache clear timestamp"
    )
...
class QueryAnalysisResponse(BaseModel):
    ...
    total_queries_analyzed: int = Field(
        ..., description="Total queries analyzed"
    )
    ...
    most_expensive_queries: list[dict[str, Any]] = Field(
...
class UserBehaviorAnalyticsResponse(BaseModel):
    ...
    most_visited_pages: list[str] = Field(
        ..., description="Most visited pages"
    )
    user_journey: list[dict[str, Any]] = Field(
        ..., description="User journey data"
    )
    conversion_events: list[dict[str, Any]] = Field(
...
class CleanupResponse(BaseModel):
    ...
    items_cleaned: int = Field(
        ..., description="Number of items cleaned"
    )
    storage_freed_mb: float = Field(
        ..., description="Storage freed in MB"
    )
