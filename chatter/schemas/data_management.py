...
class BulkOperationFilters(BaseModel):
    ...
    user_id: str | None = Field(
        None, description="Filter by user/owner ID"
    )
    ...
    dry_run: bool = Field(
        False, description="If true, only return count without deleting"
    )
...
class BulkDeleteFilteredRequest(BaseModel):
    ...
    filters: BulkOperationFilters = Field(
...
class BulkDeletePreviewResponse(BaseModel):
    ...
    filters_applied: BulkOperationFilters = Field(
