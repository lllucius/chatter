...
class CacheWarmingService:
    ...
    async def warm_analytics_cache(
        ...
        try:
            ...
            logger.info(
                f"Starting cache warming for {len(active_users)} active users"
            )

            # Process users in batches to prevent overwhelming
            user_batches = [
                active_users[
                    i : i + self.warming_config["warming_batch_size"]
                ]
                for i in range(
                    0,
                    len(active_users),
                    self.warming_config["warming_batch_size"],
                )
            ]
            ...
            for batch in user_batches:
                batch_results = await self._warm_user_batch(
                    batch, force_refresh
                )
            ...
            system_results = await self._warm_system_caches(
                force_refresh
            )
            ...
            logger.info(
                f"Cache warming completed: {total_queries_warmed} queries warmed in {duration:.2f}s"
            )
            ...
            results.update(
                {
                    ...
                    "error": str(e),
                    "duration_seconds": (
                        datetime.now(UTC) - warming_start
                    ).total_seconds(),
                }
            )
    ...
    async def _get_active_users(self) -> list[str]:
        ...
        try:
            ...
            logger.debug(
                f"Found {len(user_ids)} active users for cache warming"
            )
            return user_ids
    ...
    async def _warm_user_batch(
        self, user_ids: list[str], force_refresh: bool
    ) -> dict[str, int]:
        ...
        for user_id in user_ids:
            for time_period in self.warming_config[
                "popular_time_ranges"
            ]:
                warming_tasks.append(
                    self._warm_user_analytics(
                        user_id, time_period, force_refresh
                    )
                )

        # Execute warming tasks with concurrency control
        semaphore = asyncio.Semaphore(
            self.warming_config["max_concurrent_warmings"]
        )
    ...
    async def _warm_user_analytics(
        self, user_id: str, time_period: str, force_refresh: bool
    ) -> int:
        ...
        try:
            ...
            analytics_methods = [
                ...
                (
                    "chart_data",
                    self.analytics_service.get_chart_ready_data,
                ),
                (
                    "integrated_stats",
                    lambda uid, tr: self.analytics_service.get_integrated_dashboard_stats(
                        uid
                    ),
                ),
            ]
            ...
            for method_name, method in analytics_methods:
                try:
                    ...
                    logger.warning(
                        f"Failed to warm {method_name} for user {user_id}: {e}"
                    )
            ...
            logger.warning(
                f"Failed to warm analytics for user {user_id}, period {time_period}: {e}"
            )
            return 0

    async def _warm_system_caches(
        ...
        try:
            ...
            logger.debug(
                f"Warmed {results['queries_warmed']} system-wide caches"
            )
    ...
    async def get_cache_warming_status(self) -> dict[str, Any]:
        ...
        try:
            ...
            cache_stats = (
                await self.analytics_service.get_analytics_performance_metrics()
            )
            ...
            status = {
                ...
                "cache_performance": {
                    ...
                    "total_instances": cache_stats.get(
                        "cache_instances", 0
                    ),
                },
    ...
    async def _get_last_warming_info(self) -> dict[str, Any]:
        ...
        try:
            ...
            last_warming = await self.cache.get(
    ...
    async def invalidate_stale_cache_entries(
        ...
        try:
            ...
            for user_id in inactive_users:
                success = (
                    await self.analytics_service.invalidate_user_cache(
                        user_id
                    )
                )
                if success:
            ...
            logger.info(
                f"Invalidated cache for {invalidated_count} inactive users"
            )
            ...
            logger.error(
                f"Failed to invalidate stale cache entries: {e}"
            )
    ...
    async def optimize_cache_performance(self) -> dict[str, Any]:
        ...
        try:
            ...
            optimization_results = {
                "current_hit_rate": performance_metrics.get(
                    "cache_hit_rate", 0
                ),
                "actions_taken": [],
                "recommendations": [],
            ...
            cache_instances = performance_metrics.get(
                "cache_instances", 0
            )
            ...
            if current_hit_rate < 40:
                ...
                warming_result = await self._warm_user_batch(
                    await self._get_active_users()[
                        :5
                    ],  # Warm top 5 active users
                    force_refresh=False,
                )
