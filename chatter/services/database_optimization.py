...
class DatabaseOptimizationService:
    ...
    async def analyze_query_performance(
        ...
        try:
            ...
            test_queries = [
                (
                    "conversation_count",
                    self._test_conversation_count_query,
                ),
                (
                    "message_aggregation",
                    self._test_message_aggregation_query,
                ),
                ("daily_stats", self._test_daily_stats_query),
                ("user_activity", self._test_user_activity_query),
                (
                    "document_analytics",
                    self._test_document_analytics_query,
                ),
            ]
            ...
            for test_name, test_func in test_queries:
                try:
                    ...
                    logger.error(
                        f"Query performance test failed for {test_name}: {e}"
                    )
    ...
    async def _test_conversation_count_query(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "performance_rating": self._rate_query_performance(
                    duration_ms
                ),
                "optimization_suggestions": self._get_query_suggestions(
                    "conversation_count", duration_ms
                ),
            }
    ...
    async def _test_message_aggregation_query(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "optimization_suggestions": self._get_query_suggestions(
                    "message_aggregation", duration_ms
                ),
            }
        ...
        try:
            ...
            query = (
                select(
                    ...
                    func.max(Conversation.created_at).label(
                        "last_activity"
                    ),
                    func.count(Message.id).label("message_count"),
                )
    ...
    async def _test_document_analytics_query(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "optimization_suggestions": self._get_query_suggestions(
                    "document_analytics", duration_ms
                ),
            }
    ...
    def _get_query_suggestions(
        self, query_type: str, duration_ms: float
    ) -> list[str]:
        ...
        if duration_ms > self.very_slow_query_threshold_ms:
            ...
            suggestions.append(
                "Review query joins and consider query restructuring"
            )
            suggestions.append(
            ...
            recommendations.append(
                {
                    ...
                    "tables_affected": [
                        rec.table_name for rec in critical_indexes
                    ],
    ...
    async def get_database_health_metrics(self) -> dict[str, Any]:
        ...
        try:
            ...
            if health_metrics["query_performance"]["slow_queries"]:
                health_metrics["recommendations"][
                    "immediate_actions"
                ].append(
