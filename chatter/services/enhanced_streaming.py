"""Enhanced streaming service for token-level streaming with comprehensive workflow support."""
...
class EnhancedStreamingService:
    """Enhanced streaming service with token-level streaming and comprehensive workflow support."""

    def __init__(self):
        """Initialize enhanced streaming service."""
    ...
    async def stream_workflow_events(
        ...
        """Stream workflow events with enhanced processing.
...
async def create_enhanced_stream(
    ...
    await enhanced_streaming_service.create_stream(
...
async def stream_workflow_with_enhancements(
    stream_id: str,
    workflow_generator: AsyncGenerator[dict[str, Any], None],
    enable_heartbeat: bool = True,
    heartbeat_interval: float = 30.0
) -> AsyncGenerator[StreamingChatChunk, None]:
    """Stream workflow with all enhancements enabled.
    ...
    Yields:
        Enhanced streaming chunks
    """
    try:
        # Create base streaming generator
        base_stream = enhanced_streaming_service.stream_workflow_events(
            stream_id, workflow_generator
        )
        
        # Add heartbeat if enabled
        if enable_heartbeat:
            enhanced_stream = enhanced_streaming_service.stream_with_heartbeat(
                stream_id, base_stream, heartbeat_interval
            )
        else:
            enhanced_stream = base_stream
        ...
        await enhanced_streaming_service.end_stream(stream_id)