...
class RealTimeAnalyticsService:
    ...
    async def start_real_time_dashboard(self, user_id: str) -> None:
        ...
        if task_key in self._update_tasks:
            logger.debug(
                f"Real-time dashboard already running for user {user_id}"
            )
            return
        ...
        logger.info(
            f"Started real-time dashboard updates for user {user_id}"
        )
    ...
    async def stop_real_time_dashboard(self, user_id: str) -> None:
        ...
        if task_key in self._update_tasks:
            ...
            try:
                ...
                pass
            logger.info(
                f"Stopped real-time dashboard updates for user {user_id}"
            )
    ...
    async def _dashboard_update_loop(self, user_id: str) -> None:
        ...
        try:
            while True:
                ...
                try:
                    dashboard_stats = (
                        await self.analytics_service.get_integrated_dashboard_stats()
                    )
                    chart_data = (
                        await self.analytics_service.get_chart_ready_data()
                    )
                    ...
                    logger.error(
                        f"Error in dashboard update loop for user {user_id}: {e}"
                    )
            ...
            logger.debug(
                f"Dashboard update loop cancelled for user {user_id}"
            )
            raise

    async def _check_for_alerts(
        self, stats: IntegratedDashboardStats, user_id: str
    ) -> None:
        ...
        if hasattr(stats, 'system_performance'):
            ...
            # CPU usage alert
            if (
                hasattr(perf, 'cpu_usage')
                and perf.cpu_usage
                > self._notification_thresholds["high_cpu_usage"]
            ):
                alerts.append(
                    {
                        "type": "performance",
                        ...
                        "threshold": self._notification_thresholds[
                            "high_cpu_usage"
                        ],
                    }
                )
        ...
        if (
            cache_stats
            and cache_stats.get('hit_rate', 100)
            < self._notification_thresholds["low_cache_hit_rate"]
        ):
            alerts.append(
                {
                    "type": "cache",
    ...
    async def send_system_health_update(
        ...
        event = Event(
            ...
            data={
                "health": health_data,
                "timestamp": datetime.now(UTC).isoformat(),
                "severity": self._determine_health_severity(
                    health_data
                ),
            },
        )
    ...
    async def get_user_behavior_analytics(
        ...
        user_analytics = {
            ...
            "activity_summary": await self._calculate_user_activity(
                user_id
            ),
...
def get_real_time_analytics_service(
