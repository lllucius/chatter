...
class WorkflowDefaultsService:
    ...
    async def get_default_model_config(
        ...
        prefer_profile_type: ProfileType | None = None,
    ) -> dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                "Failed to get default model config", error=str(e)
            )
    ...
    async def get_default_node_config(
        ...
        try:
            ...
            logger.error(
                f"Failed to get default config for node type {node_type}",
                error=str(e),
            )
            return {}
    ...
    async def get_default_prompt_text(
        ...
        try:
            ...
            logger.error(
                "Failed to get default prompt text", error=str(e)
            )
            return ""
    ...
    async def _get_default_profile(
        ...
        try:
            ...
            if prefer_profile_type:
                ...
                preferred_query = query.where(
                    Profile.profile_type == prefer_profile_type
                )
                result = await self.session.execute(
                    preferred_query.limit(1)
                )
            ...
            result = await self.session.execute(
                query.order_by(Profile.created_at.desc()).limit(1)
            )
    ...
    async def _get_system_default_model_config(self) -> dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                "Failed to get system default model config",
                error=str(e),
            )
            return self._get_hardcoded_fallback_config()
