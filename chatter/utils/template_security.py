...
class SecureTemplateRenderer:
    ...
    @classmethod
    def render_secure_f_string(cls, template: str, variables: Dict[str, Any]) -> str:
        ...
        try:
            ...
            template_converted = cls._convert_f_string_to_template(template)
    ...
    @classmethod
    def render_jinja2_secure(cls, template: str, variables: Dict[str, Any]) -> str:
    ...
    @classmethod
    def render_mustache_secure(cls, template: str, variables: Dict[str, Any]) -> str:
        ...
        try:
            ...
            logger.error(f"Mustache template rendering failed: {str(e)}")
            raise ValueError(f"Template rendering failed: {str(e)}") from e

    @classmethod
    def _sanitize_variables(cls, variables: Dict[str, Any]) -> Dict[str, str]:
        ...
        for key, value in variables.items():
            ...
            if len(str_value) > cls.MAX_VARIABLE_LENGTH:
                raise ValueError(f"Variable '{key}' value too long (max {cls.MAX_VARIABLE_LENGTH} chars)")
    ...
    def _sanitize_content(cls, content: str) -> str:
        ...
        dangerous_patterns = [
            (r'<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>', ''),  # Remove script tags
    ...
    @classmethod
    def _convert_f_string_to_template(cls, f_string_template: str) -> str:
        """Convert f-string style {var} to Template style ${var}.
    ...
    @classmethod
    def validate_template_syntax(cls, template: str, template_format: str) -> Dict[str, Any]:
            ...
            if template_format == 'f-string':
                ...
                for var in variables:
                    if not cls.SAFE_VARIABLE_PATTERN.match(var):
                        result['valid'] = False
                        result['errors'].append(f"Invalid variable name: {var}")
                ...
                try:
                    ...
                    result['warnings'].append("Jinja2 not available for validation")
                except Exception as e:
                    result['valid'] = False
