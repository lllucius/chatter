...
describe('useWorkflowChat Fix', () => {
  it('should prevent "is not a function" error with proper validation', () => {
    ...
    const validateAndCallWorkflowMethod = (getSDK) => {
      const sdk = getSDK();
    ...
    expect(() => validateAndCallWorkflowMethod(mockGetSDK))
      .toThrow('getChatWorkflowTemplatesApiV1WorkflowsTemplatesChat method not available');
  });
  ...
  it('should work correctly when the method is available', () => {
    ...
    const mockGetSDK = vi.fn().mockReturnValue({
      workflows: {
        getChatWorkflowTemplatesApiV1WorkflowsTemplatesChat: vi.fn().mockResolvedValue({
  ...
  it('should handle missing workflows API', () => {
    ...
    expect(() => validateAndCallWorkflowMethod(mockGetSDK))
      .toThrow('SDK or workflows API not initialized');
  });
});