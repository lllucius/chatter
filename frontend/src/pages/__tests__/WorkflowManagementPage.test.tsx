...
describe('WorkflowManagementPage', () => {
  ...
  test('View Details button opens dialog with template information', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Template Details: Test Template')).toBeInTheDocument();
    });
  ...
  test('View Details dialog can be closed', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByText('Template Details: Test Template')).not.toBeInTheDocument();
    });
  });
});