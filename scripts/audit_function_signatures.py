    ...
    def check_compilation(self) -> List[Tuple[Path, str]]:
        ...
        for python_file in chatter_dir.rglob("*.py"):
            ...
            try:
                ...
                self.log(f"✗ {python_file.relative_to(self.base_path)}: {e}")
        
        return compilation_errors
    
    def check_known_problematic_patterns(self) -> List[Tuple[Path, int, str]]:
                    ...
                    if isinstance(node, ast.Call) and isinstance(node.func, ast.Name):
                        ...
                        if call_name in problematic_calls:
                            if call_name not in imported_names:
                                ...
                                locally_defined = any(
                                    isinstance(n, (ast.ClassDef, ast.FunctionDef)) and n.name == call_name
                                    for n in ast.walk(tree)
                                )
                                if not locally_defined:
                                    expected_module = problematic_calls[call_name]
                                    issues.append((
                                        python_file,
    ...
    def check_critical_files(self) -> List[Tuple[Path, str]]:
        ...
        for file_path in critical_files:
            ...
            try:
                ...
                # Basic checks
                if "from chatter_sdk" in content and "ChatterSDKClient" in content:
                    # Check that ChatterSDKClient is either imported or defined
                    tree = ast.parse(content, filename=str(file_path))
                    has_import = any(
                        isinstance(node, ast.ImportFrom) and 
                        node.module and "chatter_sdk" in node.module
                        for node in ast.walk(tree)
                    )
                    has_definition = any(
                        isinstance(node, ast.ClassDef) and node.name == "ChatterSDKClient"
                        for node in ast.walk(tree)
                    )
                    
                    if not (has_import or has_definition):
                        issues.append((file_path, "ChatterSDKClient used but not imported or defined"))
                
                self.log(f"✓ {file_path.relative_to(self.base_path)}")
                
            except Exception as e:
                issues.append((file_path, f"Error checking file: {e}"))
                self.log(f"✗ {file_path.relative_to(self.base_path)}: {e}")
    ...
    def run_audit(self) -> bool:
        ...
        if compilation_errors:
            ...
            for file_path, error in compilation_errors:
                print(f"  {file_path.relative_to(self.base_path)}: {error}")
        else:
        ...
        if pattern_issues:
            ...
            for file_path, line, issue in pattern_issues:
                print(f"  {file_path.relative_to(self.base_path)}:{line} - {issue}")
        else:
        ...
        if critical_issues:
            ...
            for file_path, issue in critical_issues:
                print(f"  {file_path.relative_to(self.base_path)} - {issue}")
        else:
            print("\n✅ All critical files are in good shape")
        
        # Summary
        total_issues = len(compilation_errors) + len(pattern_issues) + len(critical_issues)
        if total_issues == 0:
...
def main():
    parser = argparse.ArgumentParser(description="Audit function call signatures")
