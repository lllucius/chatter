...
app = typer.Typer(help="Database seeding operations for Chatter platform")
console = Console()
...
def seed(
    ...
    async def _seed():
        try:
            ...
            if init_first:
                console.print("🏗️  Initializing database schema...", style="blue")
                await init_database()
                console.print("✅ Database schema initialized", style="green")
            
            # Run seeding
            console.print(f"🌱 Seeding database in {mode} mode...", style="blue")
            console.print(f"   Force: {force}, Skip existing: {skip_existing}")
            if use_config:
                console.print(f"   Using configuration: {config_file or 'auto-detected'}")
            
            if use_config:
                results = await seed_database_with_config(mode, config_file, force, skip_existing)
            else:
                results = await seed_database(mode, force, skip_existing)
...
def clear(
    ...
    async def _clear():
        try:
            console.print("🗑️  Clearing all database data...", style="red")
            success = await clear_all_data(confirm=True)
            
            if success:
                console.print("✅ All data cleared successfully", style="green")
            else:
        ...
        try:
            ...
            async with DatabaseManager() as session:
                ...
                if isinstance(user_count, int) and user_count > 0:
                    console.print(f"🌱 Database appears seeded ({user_count} users)", style="green")
                else:
                    console.print("🔍 Database appears empty", style="yellow")
