import datetime
from collections.abc import Mapping
from typing import TYPE_CHECKING, Any, TypeVar, Union, cast

from attrs import define as _attrs_define
from attrs import field as _attrs_field
from dateutil.parser import isoparse

from ..types import UNSET, Unset

if TYPE_CHECKING:
    from ..models.mcp_status_response_servers_item import McpStatusResponseServersItem


T = TypeVar("T", bound="McpStatusResponse")


@_attrs_define
class McpStatusResponse:
    """Schema for MCP status response.

    Attributes:
        status (str): MCP service status
        servers (list['McpStatusResponseServersItem']): Connected servers
        last_check (Union[None, Unset, datetime.datetime]): Last health check time
        errors (Union[Unset, list[str]]): Any error messages
    """

    status: str
    servers: list["McpStatusResponseServersItem"]
    last_check: Union[None, Unset, datetime.datetime] = UNSET
    errors: Union[Unset, list[str]] = UNSET
    additional_properties: dict[str, Any] = _attrs_field(init=False, factory=dict)

    def to_dict(self) -> dict[str, Any]:
        status = self.status

        servers = []
        for servers_item_data in self.servers:
            servers_item = servers_item_data.to_dict()
            servers.append(servers_item)

        last_check: Union[None, Unset, str]
        if isinstance(self.last_check, Unset):
            last_check = UNSET
        elif isinstance(self.last_check, datetime.datetime):
            last_check = self.last_check.isoformat()
        else:
            last_check = self.last_check

        errors: Union[Unset, list[str]] = UNSET
        if not isinstance(self.errors, Unset):
            errors = self.errors

        field_dict: dict[str, Any] = {}
        field_dict.update(self.additional_properties)
        field_dict.update(
            {
                "status": status,
                "servers": servers,
            }
        )
        if last_check is not UNSET:
            field_dict["last_check"] = last_check
        if errors is not UNSET:
            field_dict["errors"] = errors

        return field_dict

    @classmethod
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        from ..models.mcp_status_response_servers_item import McpStatusResponseServersItem

        d = dict(src_dict)
        status = d.pop("status")

        servers = []
        _servers = d.pop("servers")
        for servers_item_data in _servers:
            servers_item = McpStatusResponseServersItem.from_dict(servers_item_data)

            servers.append(servers_item)

        def _parse_last_check(data: object) -> Union[None, Unset, datetime.datetime]:
            if data is None:
                return data
            if isinstance(data, Unset):
                return data
            try:
                if not isinstance(data, str):
                    raise TypeError()
                last_check_type_0 = isoparse(data)

                return last_check_type_0
            except:  # noqa: E722
                pass
            return cast(Union[None, Unset, datetime.datetime], data)

        last_check = _parse_last_check(d.pop("last_check", UNSET))

        errors = cast(list[str], d.pop("errors", UNSET))

        mcp_status_response = cls(
            status=status,
            servers=servers,
            last_check=last_check,
            errors=errors,
        )

        mcp_status_response.additional_properties = d
        return mcp_status_response

    @property
    def additional_keys(self) -> list[str]:
        return list(self.additional_properties.keys())

    def __getitem__(self, key: str) -> Any:
        return self.additional_properties[key]

    def __setitem__(self, key: str, value: Any) -> None:
        self.additional_properties[key] = value

    def __delitem__(self, key: str) -> None:
        del self.additional_properties[key]

    def __contains__(self, key: str) -> bool:
        return key in self.additional_properties
