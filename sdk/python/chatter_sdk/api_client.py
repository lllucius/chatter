...
import chatter_sdk.models
from chatter_sdk import rest
from chatter_sdk.api_response import ApiResponse
from chatter_sdk.api_response import T as ApiResponseT
from chatter_sdk.configuration import Configuration
from chatter_sdk.exceptions import ApiException, ApiValueError
from dateutil.parser import parse
from pydantic import SecretStr

RequestSerialized = tuple[
    str, str, dict[str, str], str | None, list[str]
]
...
class ApiClient:
    ...
    def __init__(
        ...
        self.client_side_validation = (
            configuration.client_side_validation
        )
    ...
    def __deserialize(self, data, klass):
        ...
        if isinstance(klass, str):
            ...
            if klass in self.NATIVE_TYPES_MAPPING:
                ...
                klass = getattr(chatter_sdk.models, klass)
    ...
    def parameters_to_url_query(self, params, collection_formats):
        ...
        for k, v in (
            ...
            if k in collection_formats:
                ...
                if collection_format == 'multi':
                    new_params.extend(
                        (k, quote(str(value))) for value in v
                    )
                else:
                    if collection_format == 'ssv':
                        delimiter = ' '
                    ...
                    new_params.append(
                        (
                            k,
                            delimiter.join(
                                quote(str(value)) for value in v
                            ),
                        )
                    )
            else:
        ...
        return "&".join(
            ["=".join(map(str, item)) for item in new_params]
        )
    ...
    def files_parameters(
        ...
        files: dict[
            str,
            str | bytes | list[str] | list[bytes] | tuple[str, bytes],
        ],
    ):
        ...
        for k, v in files.items():
            if isinstance(v, str):
                ...
                for file_param in v:
                    params.extend(
                        self.files_parameters({k: file_param})
                    )
                continue
            else:
    ...
    def update_params_for_auth(
        ...
        if request_auth:
            ...
            for auth in auth_settings:
                auth_setting = self.configuration.auth_settings().get(
                    auth
                )
                if auth_setting:
                    self._apply_auth_params(
