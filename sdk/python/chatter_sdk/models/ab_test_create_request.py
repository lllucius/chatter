...
class ABTestCreateRequest(BaseModel):
    ...
    variants: Annotated[list[TestVariant], Field(min_length=2)] = Field(
        description="Test variants"
    )
    metrics: Annotated[list[MetricType], Field(min_length=1)] = Field(
        description="Metrics to track"
    )
    duration_days: (
        Annotated[int, Field(le=365, strict=True, ge=1)] | None
    ) = Field(default=7, description="Test duration in days")
    min_sample_size: (
        Annotated[int, Field(strict=True, ge=10)] | None
    ) = Field(default=100, description="Minimum sample size")
    confidence_level: (
        Annotated[float, Field(le=0.99, strict=True, ge=0.5)]
        | Annotated[int, Field(le=0, strict=True, ge=1)]
        | None
    ) = Field(default=0.95, description="Statistical confidence level")
    target_audience: dict[str, Any] | None = None
    traffic_percentage: (
        Annotated[float, Field(le=100.0, strict=True, ge=0.1)]
        | Annotated[int, Field(le=100, strict=True, ge=1)]
        | None
    ) = Field(
        default=100.0, description="Percentage of traffic to include"
    )
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        if (
            self.target_audience is None
            and "target_audience" in self.model_fields_set
        ):
        ...
        _obj = cls.model_validate(
            {
                ...
                "duration_days": (
                    obj.get("duration_days")
                    if obj.get("duration_days") is not None
                    else 7
                ),
                "min_sample_size": (
                    obj.get("min_sample_size")
                    if obj.get("min_sample_size") is not None
                    else 100
                ),
                "confidence_level": (
                    obj.get("confidence_level")
                    if obj.get("confidence_level") is not None
                    else 0.95
                ),
                "target_audience": obj.get("target_audience"),
                "traffic_percentage": (
                    obj.get("traffic_percentage")
                    if obj.get("traffic_percentage") is not None
                    else 100.0
                ),
                "tags": obj.get("tags"),
