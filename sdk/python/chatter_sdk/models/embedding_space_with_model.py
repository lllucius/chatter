...
class EmbeddingSpaceWithModel(BaseModel):
    ...
    ] = Field(description="Unique space name")
    ...
    base_dimensions: Annotated[int, Field(le=10000, strict=True)] = (
        Field(description="Original model dimensions")
    )
    effective_dimensions: Annotated[
        int, Field(le=10000, strict=True)
    ] = Field(description="Effective dimensions after reduction")
    ...
    normalize_vectors: StrictBool | None = Field(
        default=True, description="Whether to normalize vectors"
    )
    ...
    ) = Field(default='hnsw', description="Index type")
    index_config: dict[str, Any] | None = Field(
        default=None, description="Index configuration"
    )
    is_active: StrictBool | None = Field(
        default=True, description="Whether space is active"
    )
    is_default: StrictBool | None = Field(
        default=False, description="Whether this is the default space"
    )
    id: StrictStr
    model_id: StrictStr
    created_at: datetime
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # and model_fields_set contains the field
        if (
            self.reducer_path is None
            and "reducer_path" in self.model_fields_set
        ):
        ...
        # and model_fields_set contains the field
        if (
            self.reducer_version is None
            and "reducer_version" in self.model_fields_set
        ):
        ...
        _obj = cls.model_validate(
            {
                ...
                "normalize_vectors": (
                    obj.get("normalize_vectors")
                    if obj.get("normalize_vectors") is not None
                    else True
                ),
                ...
                "index_type": (
                    obj.get("index_type")
                    if obj.get("index_type") is not None
                    else 'hnsw'
                ),
                ...
                "model": (
                    ModelDefWithProvider.from_dict(obj["model"])
