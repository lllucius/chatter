...
class ModelDefWithProvider(BaseModel):
    ...
    model_name: Annotated[
        str, Field(min_length=1, strict=True, max_length=200)
    ] = Field(description="Actual model name for API calls")
    max_tokens: (
        Annotated[int, Field(le=1000000, strict=True)] | None
    ) = None
    context_length: (
        Annotated[int, Field(le=10000000, strict=True)] | None
    ) = None
    dimensions: Annotated[int, Field(le=10000, strict=True)] | None = (
        None
    )
    chunk_size: Annotated[int, Field(le=100000, strict=True)] | None = (
        None
    )
    supports_batch: StrictBool | None = Field(
        default=False,
        description="Whether model supports batch operations",
    )
    max_batch_size: (
        Annotated[int, Field(le=10000, strict=True)] | None
    ) = None
    default_config: dict[str, Any] | None = Field(
        default=None, description="Default configuration"
    )
    is_active: StrictBool | None = Field(
        default=True, description="Whether model is active"
    )
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # and model_fields_set contains the field
        if (
            self.context_length is None
            and "context_length" in self.model_fields_set
        ):
        ...
        if (
            ...
            and "dimensions" in self.model_fields_set
        ):
        ...
        if (
            ...
            and "chunk_size" in self.model_fields_set
        ):
        ...
        if (
            ...
            and "max_batch_size" in self.model_fields_set
        ):
        ...
        _obj = cls.model_validate(
            {
                ...
                "supports_batch": (
                    obj.get("supports_batch")
                    if obj.get("supports_batch") is not None
                    else False
                ),
                ...
                "provider": (
                    Provider.from_dict(obj["provider"])
