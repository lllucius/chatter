...
class RestoreRequest(BaseModel):
    ...
    backup_id: StrictStr = Field(
        description="Backup ID to restore from"
    )
    restore_options: dict[str, Any] | None = Field(
        default=None, description="Restore options"
    )
    create_backup_before_restore: StrictBool | None = Field(
        default=True, description="Create backup before restore"
    )
    verify_integrity: StrictBool | None = Field(
        default=True,
        ...
        _obj = cls.model_validate(
            {
                ...
                "create_backup_before_restore": (
                    obj.get("create_backup_before_restore")
                    if obj.get("create_backup_before_restore")
                    is not None
                    else True
                ),
                "verify_integrity": (
                    obj.get("verify_integrity")
                    if obj.get("verify_integrity") is not None
                    else True
                ),
            }
        )
