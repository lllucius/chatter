...
class SystemAnalyticsResponse(BaseModel):
    ...
    active_users_today: StrictInt = Field(
        description="Active users today"
    )
    active_users_week: StrictInt = Field(
        description="Active users this week"
    )
    active_users_month: StrictInt = Field(
        description="Active users this month"
    )
    system_uptime_seconds: StrictFloat | StrictInt = Field(
        description="System uptime in seconds"
    )
    avg_cpu_usage: StrictFloat | StrictInt = Field(
        description="Average CPU usage percentage"
    )
    avg_memory_usage: StrictFloat | StrictInt = Field(
        description="Average memory usage percentage"
    )
    database_connections: StrictInt = Field(
        description="Current database connections"
    )
    total_api_requests: StrictInt = Field(
        description="Total API requests"
    )
    requests_per_endpoint: dict[str, StrictInt] = Field(
        description="Requests by endpoint"
    )
    avg_api_response_time: StrictFloat | StrictInt = Field(
        description="Average API response time"
    )
    api_error_rate: StrictFloat | StrictInt = Field(
        description="API error rate"
    )
    storage_usage_bytes: StrictInt = Field(
        description="Total storage usage"
    )
    vector_database_size_bytes: StrictInt = Field(
        description="Vector database size"
    )
    cache_hit_rate: StrictFloat | StrictInt = Field(
        ...
        _obj = cls.model_validate(
            {
                ...
                "requests_per_endpoint": obj.get(
                    "requests_per_endpoint"
                ),
                "avg_api_response_time": obj.get(
                    "avg_api_response_time"
                ),
                "api_error_rate": obj.get("api_error_rate"),
                "storage_usage_bytes": obj.get("storage_usage_bytes"),
                "vector_database_size_bytes": obj.get(
                    "vector_database_size_bytes"
                ),
