...
class TestAuthE2E:
    ...
    async def test_complete_login_workflow(
        self, e2e_client: AsyncClient, e2e_test_user: dict[str, Any]
    ):
    ...
    async def test_profile_access_workflow(
        ...
        if not e2e_auth_headers:
            pytest.skip(
                "Authentication failed - cannot test profile access"
            )

        try:
            # Access profile
            response = await e2e_client.get(
                "/api/v1/auth/profile", headers=e2e_auth_headers
            )
    ...
    async def test_registration_login_profile_complete_flow(
        ...
        try:
            ...
            reg_response = await e2e_client.post(
                "/api/v1/auth/register", json=unique_user_data
            )
            ...
            pytest.skip(
                f"Complete authentication flow test skipped: {e}"
            )
    ...
    async def test_invalid_credentials_workflow(
        ...
        try:
            ...
            response = await e2e_client.post(
                "/api/v1/auth/login", json=invalid_login
            )
            ...
            pytest.skip(
                f"Invalid credentials workflow test skipped: {e}"
            )
