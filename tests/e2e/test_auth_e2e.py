...
class TestAuthenticationE2E:
    ...
    def test_complete_user_registration_flow(
        self, test_client, sample_user_credentials, cleanup_test_data
    ):
        ...
        if register_response.status_code == status.HTTP_404_NOT_FOUND:
            pytest.skip(
                "Registration endpoint not available in test environment"
            )

        # Step 3: Verify registration success
        assert (
            "user_id" in register_response.json()
            or "id" in register_response.json()
        )
        ...
        if login_response.status_code == status.HTTP_404_NOT_FOUND:
            pytest.skip(
                "Login endpoint not available in test environment"
            )
    ...
    def test_password_reset_flow(
        ...
        if (
            reset_request_response.status_code
            == status.HTTP_404_NOT_FOUND
        ):
            pytest.skip(
                "Password reset endpoint not available in test environment"
            )
