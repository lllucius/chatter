...
class TestChatConversationE2E:
    ...
    def test_complete_conversation_lifecycle(self, test_client, sample_chat_conversation, cleanup_test_data):
        ...
        if not conversation_id:
            pytest.skip("Unable to extract conversation ID from response")
        
        # Step 2: Add messages to the conversation
        message_response = test_client.post(
            f"/api/v1/chat/conversations/{conversation_id}/messages",
            json={
                "content": sample_chat_conversation["messages"][0]["content"],
                "role": "user"
            }
        )
        
        # Allow for endpoint not existing or auth required
        if message_response.status_code not in [status.HTTP_200_OK, status.HTTP_201_CREATED]:
            pytest.skip("Unable to add messages to conversation")
        
        # Step 3: Get conversation with messages
        get_response = test_client.get(f"/api/v1/chat/conversations/{conversation_id}")
        
        if get_response.status_code == status.HTTP_200_OK:
            conversation = get_response.json()
            assert conversation["title"] == sample_chat_conversation["title"]
    ...
    def test_chat_message_with_ai_response(self, mock_llm, test_client, mock_llm_response):
        ...
        if create_response.status_code not in [status.HTTP_200_OK, status.HTTP_201_CREATED]:
            pytest.skip("Cannot create conversation for AI response test")
    ...
    def test_conversation_listing_and_pagination(self, test_client):
        ...
        if list_response.status_code == status.HTTP_401_UNAUTHORIZED:
            pytest.skip("Authentication required for conversation listing")
        
        assert list_response.status_code == status.HTTP_200_OK
        conversations = list_response.json()
        
        # Test pagination if supported
        paginated_response = test_client.get("/api/v1/chat/conversations?page=1&limit=10")
...
class TestChatStreamingE2E:
    ...
    def test_streaming_chat_response(self, test_client):
        ...
        if stream_response.status_code == status.HTTP_200_OK:
            ...
            assert "stream" in content_type or "event" in content_type or "json" in content_type
    ...
    def test_websocket_chat_connection(self, test_client):
        ...
        assert websocket_response.status_code != status.HTTP_404_NOT_FOUND