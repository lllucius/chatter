...
class TestChatE2E:
    ...
    async def test_complete_chat_conversation_workflow(
        ...
        if not e2e_auth_headers:
            pytest.skip(
                "Authentication failed - cannot test chat workflow"
            )
        ...
        try:
            ...
            user_messages = [
                msg for msg in messages if msg["message_type"] == "user"
            ]
    ...
    async def test_chat_list_and_retrieval_workflow(
        ...
        try:
            ...
            list_response = await e2e_client.get(
                "/api/v1/chats", headers=e2e_auth_headers
            )
            ...
            for chat_id in chat_ids:
                get_response = await e2e_client.get(
                    f"/api/v1/chats/{chat_id}", headers=e2e_auth_headers
                )

                if get_response.status_code == 404:
                    pytest.skip(
                        "Individual chat retrieval endpoint not implemented yet"
                    )
            ...
            pytest.skip(
                f"Chat list and retrieval workflow test skipped: {e}"
            )
    ...
    async def test_chat_message_streaming_workflow(
        ...
        try:
            ...
            chat_data = {
                ...
                "description": "Test streaming messages",
            }
            ...
            if stream_response.status_code == 404:
                pytest.skip(
                    "Chat streaming endpoint not implemented yet"
                )
    ...
    async def test_chat_deletion_workflow(
        ...
        try:
            ...
            message_data = {
                "message": "This message will be deleted with the chat",
                "message_type": "user",
            }
            ...
            if delete_response.status_code == 404:
                pytest.skip(
                    "Chat deletion endpoint not implemented yet"
                )
    ...
    async def test_multi_user_chat_isolation(
        self, e2e_client: AsyncClient, test_user_data: dict[str, Any]
    ):
        ...
        try:
            ...
            for user_data in [user1_data, user2_data]:
                reg_response = await e2e_client.post(
            ...
            for user_data in [user1_data, user2_data]:
                ...
                if login_response.status_code == 404:
                    pytest.skip(
                        "User login endpoint not implemented yet"
                    )
            ...
            user1_chat_response = await e2e_client.post(
                "/api/v1/chats",
                json=user1_chat_data,
            ...
            user2_chat_response = await e2e_client.post(
                "/api/v1/chats",
                json=user2_chat_data,
                headers=user2_headers,
            )
            user2_chat_id = user2_chat_response.json()["id"]

            # Verify isolation: User 1 should not access User 2's chat
            user1_access_user2_chat = await e2e_client.get(
                f"/api/v1/chats/{user2_chat_id}", headers=user1_headers
            )
            assert user1_access_user2_chat.status_code in [
                403,
                404,
            ]  # Should be forbidden or not found

            # Verify isolation: User 2 should not access User 1's chat
            user2_access_user1_chat = await e2e_client.get(
                f"/api/v1/chats/{user1_chat_id}", headers=user2_headers
            )
            assert user2_access_user1_chat.status_code in [
                403,
                404,
            ]  # Should be forbidden or not found
