...
class BaseChatterUser(HttpUser):
    ...
    def safe_request(self, method: str, url: str, **kwargs):
        ...
        with getattr(self.client, method.lower())(url, catch_response=True, **kwargs) as response:
            if response.status_code == 404:
                response.success()  # Don't count missing endpoints as failures
                return None
            return response
    
    def create_test_chat(self) -> Optional[Dict[str, Any]]:
        ...
        response = self.safe_request("post", "/api/v1/chats", json=chat_data, headers=self.auth_headers)
    ...
    def create_test_document(self) -> Optional[Dict[str, Any]]:
        """Create a test document and return its data."""
        if not self.auth_headers:
            return None
        
        # Create a simple text file
        content = f"Load test document content {random.randint(1, 1000)} " * 20
    ...
    def send_chat_message(self, chat_id: str, message: str) -> bool:
        ...
        response = self.safe_request("post", f"/api/v1/chats/{chat_id}/messages", json=message_data, headers=self.auth_headers)
...
class AuthenticationMixin:
    ...
    def get_profile(self):
        ...
        if self.auth_headers:
            self.safe_request("get", "/api/v1/auth/profile", headers=self.auth_headers)
    
    @task(1)
    def refresh_token(self):
        """Attempt to refresh authentication token."""
        if self.auth_headers:
            self.safe_request("post", "/api/v1/auth/refresh", headers=self.auth_headers)
...
class ChatMixin:
    ...
    def list_chats(self):
        """List user's chats."""
        if self.auth_headers:
            self.safe_request("get", "/api/v1/chats", headers=self.auth_headers)
