...
from tests.load.locust_base import (
    ChatLoadTestUser,
    DocumentLoadTestUser, 
    HealthCheckLoadTestUser,
    ChatterUser
)
...
class MixedWorkloadUser(ChatterUser):
    ...
    def browse_conversations(self):
        ...
        self.client.get("/api/v1/chat/conversations", headers=self.get_headers(), catch_response=True)
    ...
    def send_chat_message(self):
        ...
        if self.conversation_ids:
            ...
            self.client.post(
                ...
                json={"content": "Hello from load test", "role": "user"},
                headers=self.get_headers(),
                catch_response=True
            )
...
class HeavyUser(ChatterUser):
    ...
    def upload_large_document(self):
        ...
        large_content = "Large document content for load testing.\n" * 100
        self.client.post(
    ...
    def analytics_request(self):
        ...
        self.client.get(endpoint, headers=self.get_headers(), catch_response=True)
...
if __name__ == "__main__":
    ...
    print("locust -f tests/load/locust_scenarios.py --host=http://localhost:8000")
    print("\nAvailable user types:")
    print("- MixedWorkloadUser: Realistic mix of operations (weight: 3)")
