...
class HeavyUser(BaseChatterUser, ChatMixin, DocumentMixin):
    ...
    def upload_large_documents(self):
        ...
        large_content = (
            "Large document content for load testing. " * 500
        )  # ~20KB
        files = {
            "file": (
                "large_loadtest_doc.txt",
                large_content,
                "text/plain",
            )
        }
    ...
    def send_many_messages(self):
        ...
        if self.chats and self.auth_headers:
            ...
            for i in range(random.randint(5, 10)):
                message = (
                    f"Rapid message {i+1}: {random.randint(1, 10000)}"
                )
                self.send_chat_message(chat["id"], message)
    ...
    def complex_search_operations(self):
        ...
        if self.auth_headers:
            ...
            self.safe_request(
                "get",
                f"/api/v1/documents/search?q={term}",
                headers=self.auth_headers,
            )
    ...
    def manage_chats(self):
        ...
        if random.random() < 0.7:  # 70% create new chat
            ...
            if len(self.chats) > 3:  # Keep at least 3 chats
                chat_to_delete = self.chats.pop(
                    random.randint(0, len(self.chats) - 1)
                )
                self.safe_request(
                    "delete",
                    f"/api/v1/chats/{chat_to_delete['id']}",
                    headers=self.auth_headers,
                )
