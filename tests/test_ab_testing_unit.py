...
from chatter.schemas.ab_testing import TestStatus, ABTestResponse
...
class TestABTestingUnit:
    ...
    async def test_get_ab_test_success(self, mock_get_manager, client: AsyncClient, auth_headers: dict):
        ...
        response = await client.get("/api/v1/ab-tests/test-123", headers=auth_headers)
        assert response.status_code == 200
    ...
    async def test_start_ab_test_success(self, mock_get_manager, client: AsyncClient, auth_headers: dict):
        ...
        response = await client.post("/api/v1/ab-tests/test-123/start", headers=auth_headers)
        assert response.status_code == 200
    ...
    async def test_delete_running_test_forbidden(self, mock_get_manager, client: AsyncClient, auth_headers: dict):
        ...
        response = await client.delete("/api/v1/ab-tests/test-123", headers=auth_headers)
        assert response.status_code == 400
    ...
    async def test_get_results_success(self, mock_get_manager, client: AsyncClient, auth_headers: dict):
        ...
        mock_results = {
            ...
            "status": "completed",
            "results": {
                "control": {"conversions": 100, "conversion_rate": 10.0},
                "variant_a": {"conversions": 120, "conversion_rate": 12.0}
            },
            "winner": "variant_a",
            "confidence": 95.0
        }
        ...
        response = await client.get("/api/v1/ab-tests/test-123/results", headers=auth_headers)
        assert response.status_code == 200
    ...
    async def test_get_metrics_success(self, mock_get_manager, client: AsyncClient, auth_headers: dict):
        ...
        response = await client.get("/api/v1/ab-tests/test-123/metrics", headers=auth_headers)
        assert response.status_code == 200
