...
from chatter.core.agents import AgentManager, AgentType, AgentStatus
from chatter.schemas.agents import AgentProfile
...
class TestAgentFeatureCompletion:
    ...
    async def test_agent_creation_with_fake_llm(self):
        ...
        # Create mock LLM
        mock_llm = AsyncMock()
        mock_llm.ainvoke = AsyncMock(return_value=AsyncMock(content="Test response"))
    ...
    async def test_agent_listing_and_filtering(self):
        ...
        spec_agent_id = await manager.create_agent(
        ...
        conv_agents, conv_total = await manager.list_agents(agent_type=AgentType.CONVERSATIONAL)
        assert conv_total >= 1
        
        spec_agents, spec_total = await manager.list_agents(agent_type=AgentType.SPECIALIST)
        assert spec_total >= 1
        
        # Test filtering by user
        user1_agents, user1_total = await manager.list_agents(user_id="user1")
    ...
    async def test_agent_statistics(self):
        ...
        # Create mock LLM
        mock_llm = AsyncMock()
        mock_llm.ainvoke = AsyncMock(return_value=AsyncMock(content="Test"))
    ...
    def test_agent_manager_class_mapping(self):
        ...
        for agent_type, agent_class in manager.agent_classes.items():
