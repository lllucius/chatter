...
class TestAgentsIntegration:
    ...
    async def test_agent_complete_lifecycle(self, client: AsyncClient, auth_headers: dict, db_session: AsyncSession):
        ...
        update_response = await client.put(f"/api/v1/agents/{agent_id}", json=update_data, headers=auth_headers)
        ...
        test_response = await client.post(f"/api/v1/agents/{agent_id}/test", json=test_data, headers=auth_headers)
        ...
        # Check agent health
        health_response = await client.get(f"/api/v1/agents/{agent_id}/health", headers=auth_headers)
        ...
        # Delete agent
        delete_response = await client.delete(f"/api/v1/agents/{agent_id}", headers=auth_headers)
        assert delete_response.status_code == 200
        
        # Verify deletion
        get_deleted_response = await client.get(f"/api/v1/agents/{agent_id}", headers=auth_headers)
    ...
    async def test_agents_list_and_filter(self, client: AsyncClient, auth_headers: dict, db_session: AsyncSession):
        ...
        # Test type filtering
        chat_response = await client.get("/api/v1/agents/?agent_type=chat", headers=auth_headers)
        ...
        # Test pagination
        page_response = await client.get("/api/v1/agents/?page=1&per_page=2", headers=auth_headers)
    ...
    async def test_agent_templates_workflow(self, client: AsyncClient, auth_headers: dict, db_session: AsyncSession):
        ...
        if len(templates) > 0:
            ...
            create_response = await client.post("/api/v1/agents/", json=agent_from_template, headers=auth_headers)
            assert create_response.status_code == 201
    ...
    async def test_bulk_agent_operations(self, client: AsyncClient, auth_headers: dict, db_session: AsyncSession):
        ...
        assert bulk_result["created_count"] >= 2  # At least 2 should succeed
        
        created_agent_ids = [agent["id"] for agent in bulk_result["created_agents"]]
        ...
        bulk_delete_response = await client.delete("/api/v1/agents/bulk", json=bulk_delete_data, headers=auth_headers)
        assert bulk_delete_response.status_code == 200
        
        delete_result = bulk_delete_response.json()
        assert delete_result["deleted_count"] >= len(created_agent_ids) - 1  # Allow for some failures
    ...
    async def test_agent_health_monitoring(self, client: AsyncClient, auth_headers: dict, db_session: AsyncSession):
        ...
        if health_response.status_code == 200:
            ...
            optional_fields = ["last_active", "response_time", "error_rate", "uptime", "checks"]
    ...
    async def test_agent_data_validation(self, client: AsyncClient, auth_headers: dict, db_session: AsyncSession):
        ...
        response = await client.post("/api/v1/agents/", json=incomplete_data, headers=auth_headers)
        assert response.status_code == 422  # Validation error
        ...
        response = await client.post("/api/v1/agents/", json=invalid_config_data, headers=auth_headers)
        assert response.status_code == 422  # Validation error
    ...
    async def test_concurrent_agent_operations(self, client: AsyncClient, auth_headers: dict, db_session: AsyncSession):
        ...
        for agent_id in created_agent_ids:
            operation_tasks.append(
                asyncio.create_task(client.get(f"/api/v1/agents/{agent_id}", headers=auth_headers))
            )
        
        # Also get templates and list agents concurrently
        operation_tasks.extend([
            asyncio.create_task(client.get("/api/v1/agents/templates", headers=auth_headers)),
            asyncio.create_task(client.get("/api/v1/agents/", headers=auth_headers))
        ])
        ...
        cleanup_tasks = [
            asyncio.create_task(client.delete(f"/api/v1/agents/{agent_id}", headers=auth_headers))
            for agent_id in created_agent_ids
        ]
    ...
    async def test_agent_complex_configuration(self, client: AsyncClient, auth_headers: dict, db_session: AsyncSession):
        ...
        assert retrieved_agent["configuration"]["temperature"] == complex_agent_data["configuration"]["temperature"]
        assert retrieved_agent["configuration"]["max_tokens"] == complex_agent_data["configuration"]["max_tokens"]
        ...
        update_response = await client.put(f"/api/v1/agents/{agent_id}", json=complex_update_data, headers=auth_headers)
        assert update_response.status_code == 200
        
        # Verify complex update
        final_get_response = await client.get(f"/api/v1/agents/{agent_id}", headers=auth_headers)
