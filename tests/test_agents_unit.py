...
class TestAgentsUnit:
    ...
    @pytest.mark.unit
    async def test_create_agent_requires_auth(
    ...
    @pytest.mark.unit
    async def test_get_agent_templates_requires_auth(
        self, client: AsyncClient
    ):
    ...
    @pytest.mark.unit
    async def test_update_agent_requires_auth(
        self, client: AsyncClient
    ):
    ...
    @pytest.mark.unit
    async def test_delete_agent_requires_auth(
        self, client: AsyncClient
    ):
    ...
    async def test_test_agent_requires_auth(self, client: AsyncClient):
        ...
        response = await client.post(
            "/api/v1/agents/agent-id/test", json={}
        )
        assert response.status_code == 401

    @pytest.mark.unit
    async def test_get_agent_health_requires_auth(
        self, client: AsyncClient
    ):
    ...
    @pytest.mark.unit
    async def test_bulk_create_agents_requires_auth(
        self, client: AsyncClient
    ):
    ...
    @pytest.mark.unit
    async def test_bulk_delete_agents_requires_auth(
        self, client: AsyncClient
    ):
    ...
    async def test_get_agent_success(
        ...
        response = await client.get(
            "/api/v1/agents/agent-123", headers=auth_headers
        )
        assert response.status_code == 200
    ...
    async def test_get_agent_not_found(
        ...
        response = await client.get(
            "/api/v1/agents/nonexistent", headers=auth_headers
        )
    ...
    async def test_test_agent_success(
        ...
        mock_test_result = {
            ...
            "response_time": 1.25,
            "success": True,
            "metadata": {"tokens_used": 25, "model_used": "gpt-4"},
        }
    ...
    async def test_get_agent_health_success(
        ...
        response = await client.get(
            "/api/v1/agents/agent-123/health", headers=auth_headers
        )
        assert response.status_code == 200
    ...
    async def test_bulk_create_agents_success(
        ...
        response = await client.post(
            "/api/v1/agents/bulk", json=bulk_data, headers=auth_headers
        )
        assert response.status_code == 201
    ...
    async def test_agent_service_error_handling(
        ...
        mock_service = AsyncMock()
        mock_service.create_agent.side_effect = Exception(
            "Agent service error"
        )
        mock_agent_service.return_value = mock_service
    ...
    async def test_agent_test_with_invalid_input(
        ...
        mock_service = AsyncMock()
        mock_service.test_agent.side_effect = ValueError(
            "Invalid test input"
        )
        mock_agent_service.return_value = mock_service
