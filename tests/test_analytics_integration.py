...
class TestAnalyticsIntegration:
    ...
    async def test_concurrent_analytics_requests(self, client: AsyncClient, auth_headers: dict, db_session: AsyncSession):
        ...
        for response in responses:
            assert response.status_code in [200, 500, 503]  # Success or graceful error
    ...
    async def test_analytics_data_consistency(self, client: AsyncClient, auth_headers: dict, db_session: AsyncSession):
        ...
        if (dashboard_response.status_code == 200 and 
            usage_response.status_code == 200 and 
            ...
            dashboard_api_calls = dashboard_data.get("summary", {}).get("api_calls_today")
            usage_api_calls = usage_data.get("api_calls_today")
            summary_api_calls = summary_data.get("api_calls")
            
            # They should be consistent if all are provided
            if all(x is not None for x in [dashboard_api_calls, usage_api_calls]):
    ...
    async def test_analytics_error_handling(self, client: AsyncClient, auth_headers: dict, db_session: AsyncSession):
        ...
        invalid_export_response = await client.post("/api/v1/analytics/export", json=invalid_export_data, headers=auth_headers)
