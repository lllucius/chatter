...
class TestAsyncLangGraphMethods:
    ...
    @pytest.mark.asyncio
    async def test_async_methods_can_be_called_in_running_event_loop(self):
    ...
    async def test_get_retriever_with_successful_embeddings(self):
        ...
        with mock.patch('chatter.services.embeddings.EmbeddingService') as mock_embedding_service, \
             mock.patch('chatter.core.vector_store.vector_store_manager') as mock_vector_store_manager:
            
            mock_service_instance = AsyncMock()
            mock_embedding_service.return_value = mock_service_instance
            mock_service_instance.get_default_provider = AsyncMock(return_value=mock_embeddings)
            
            mock_vector_store_manager.create_store.return_value = mock_vector_store
            mock_vector_store.as_retriever.return_value = mock_retriever
            
            retriever = await manager.get_retriever("test_workspace", document_ids=["doc1", "doc2"])
