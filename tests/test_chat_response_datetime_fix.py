...
class TestChatResponseDatetimeSerialization:
    ...
    def test_message_response_datetime_serialization(self):
        ...
        assert isinstance(message_dict['created_at'], str), "created_at should be serialized as string"
        assert message_dict['created_at'] == now.isoformat(), "created_at should be in ISO format"
    ...
    def test_conversation_response_datetime_serialization(self):
        ...
        assert isinstance(conv_dict['created_at'], str), "created_at should be serialized as string"
        assert isinstance(conv_dict['updated_at'], str), "updated_at should be serialized as string"
    ...
    def test_chat_response_full_serialization(self):
        ...
        parsed = json.loads(json_str)
        assert parsed['conversation_id'] == "01JJ0CONVERSATION12345ABCDEF"
        assert parsed['message']['content'] == "This is a test response from the AI assistant."
    ...
    def test_json_serialization_round_trip(self):
        ...
        recovered_message = MockMessageResponse(
            ...
            created_at=datetime.fromisoformat(parsed_data['message']['created_at'])
        )
        ...
        recovered_conversation = MockConversationResponse(
            ...
            enable_retrieval=parsed_data['conversation']['enable_retrieval'],
            context_window=parsed_data['conversation']['context_window'],
            memory_enabled=parsed_data['conversation']['memory_enabled'],
            retrieval_limit=parsed_data['conversation']['retrieval_limit'],
            retrieval_score_threshold=parsed_data['conversation']['retrieval_score_threshold'],
            created_at=datetime.fromisoformat(parsed_data['conversation']['created_at']),
            updated_at=datetime.fromisoformat(parsed_data['conversation']['updated_at'])
        )
        ...
        assert recovered_chat.conversation_id == original_chat.conversation_id
        assert recovered_chat.message.content == original_chat.message.content
        assert recovered_chat.conversation.title == original_chat.conversation.title
        assert recovered_chat.message.created_at == original_chat.message.created_at
        assert recovered_chat.conversation.created_at == original_chat.conversation.created_at