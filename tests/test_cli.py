...
class TestAPIClient:
    ...
    def test_api_client_init_custom_values(self):
        ...
        client = APIClient(base_url=custom_url, access_token=custom_token)
    ...
    async def test_request_success(self):
        ...
        client = APIClient(base_url="https://api.test.com", access_token="token")
    ...
    async def test_request_connection_error(self):
        ...
        with patch.object(client.client, 'request', new_callable=AsyncMock) as mock_request:
            ...
            with patch('chatter.cli.console') as mock_console:
                ...
                mock_console.print.assert_any_call("❌ Connection error: Connection failed")
                mock_console.print.assert_any_call("💡 Make sure the Chatter server is running.")


class TestGetAPIClient:
    """Test get_api_client function."""

    @patch.dict(os.environ, {"CHATTER_ACCESS_TOKEN": "env_token", "CHATTER_BASE_URL": "https://env.api.com"})
    ...
    def test_get_api_client_defaults(self):
        ...
        expected_url = f"http://{settings.host}:{settings.port}"
        assert client.base_url == expected_url
        assert client.access_token is None or client.access_token == settings.chatter_access_token
...
class TestCLICommands:
    ...
    def test_cli_app_help(self):
        ...
        assert "Chatter - Advanced AI Chatbot Backend API Platform" in result.stdout
    ...
    def test_list_prompts_command(self, mock_get_client, mock_asyncio_run):
        """Test list prompts command."""
        mock_client = MagicMock()
        mock_get_client.return_value = mock_client
        
        result = self.runner.invoke(app, ["prompts", "list", "--limit", "10"])
        assert result.exit_code == 0
        mock_asyncio_run.assert_called_once()

    @patch('chatter.cli.asyncio.run')
    @patch('chatter.cli.get_api_client')
    def test_create_prompt_command(self, mock_get_client, mock_asyncio_run):
    ...
    @patch('chatter.cli.asyncio.run')
    @patch('chatter.cli.get_api_client') 
    def test_delete_prompt_command(self, mock_get_client, mock_asyncio_run):
...
class TestDatabaseCommands:
    ...
    async def test_db_init_init_failure(self, mock_check_db, mock_init_db):
        ...
        with patch('chatter.cli.console') as mock_console:
            ...
            mock_check_db.assert_called_once()
            mock_init_db.assert_called_once()
            mock_console.print.assert_any_call("❌ Database initialization failed: Init failed")
...
class TestCLIIntegration:
    ...
    def test_full_prompt_lifecycle(self, mock_get_client):
        ...
        # Delete prompt
        result = self.runner.invoke(app, ["prompts", "delete", "new_prompt_id"])
        assert result.exit_code == 0
