...
from chatter.schemas.auth import UserRegistration, UserUpdate, UserResponse
...
class TestComprehensiveSchemaModelConsistency:
    ...
    def test_profile_schema_model_consistency(self):
        ...
        assert not missing_critical, f"Critical fields missing in ProfileResponse: {missing_critical}"
    ...
    def test_prompt_schema_model_consistency(self):
        ...
        missing_in_model = [f for f in update_fields if f not in model_fields]
        assert not missing_in_model, f"PromptUpdate fields missing in Prompt model: {missing_in_model}"
    ...
    def test_registry_models_schema_consistency(self):
        ...
        missing_in_model = [f for f in embedding_create_fields if f not in embedding_model_fields]
        assert not missing_in_model, f"EmbeddingSpaceCreate fields missing in EmbeddingSpace model: {missing_in_model}"
    ...
    def test_no_reserved_field_names(self):
        ...
        for model in models_to_check:
            if hasattr(model, '__table__'):
                ...
                assert not problematic_fields, f"{model.__name__} uses reserved field names: {problematic_fields}"
    ...
    def test_required_model_fields_covered_in_creation_schemas(self):
        ...
        test_cases = [
            (User, UserRegistration, {'hashed_password': 'password'}),  # Maps password to hashed_password
            (Conversation, ConversationCreate, {'user_id': None}),  # user_id comes from auth context
            (Document, DocumentCreate, {
                # Document fields that come from file processing, not user input
                'owner_id': None, 'filename': None, 'original_filename': None,
                'file_size': None, 'file_hash': None, 'mime_type': None, 'document_type': None
            }),
            (Profile, ProfileCreate, {'owner_id': None}),  # owner_id comes from auth context
            (Prompt, PromptCreate, {'owner_id': None, 'content_hash': None}),  # owner_id comes from auth, content_hash is derived
        ...
        for model_class, create_schema_class, field_mappings in test_cases:
            ...
            for field in required_fields:
                ...
                if field in field_mappings and field_mappings[field] is None:
                    continue
