...
class TestAPIContractValidation:
    ...
    def _validate_schema(
        ...
        if schema["type"] == "object":
            assert isinstance(
                data, dict
            ), f"{endpoint}: Expected object, got {type(data)}"
            ...
            for field, field_schema in properties.items():
                if field in data:
                    ...
                    if expected_type == "string":
                        assert isinstance(
                            actual_value, str
                        ), f"{endpoint}: {field} should be string"
                    elif expected_type == "number":
                        assert isinstance(
                            actual_value, (int, float)
                        ), f"{endpoint}: {field} should be number"
                    elif expected_type == "boolean":
                        assert isinstance(
                            actual_value, bool
                        ), f"{endpoint}: {field} should be boolean"
        ...
        for endpoint in v1_endpoints:
            ...
            if response.status_code not in [404, 405]:
                ...
                if response.status_code < 400:
                    ...
                    ), f"Missing content-type for {endpoint}"
...
class TestAPIDocumentationCompliance:
    ...
    def test_api_documentation_endpoints(self, test_client):
        ...
        assert (
            len(available_docs) > 0
        ), "No API documentation endpoints available"
        ...
        for endpoint, method in frontend_critical_endpoints:
            ...
            if response.status_code == 200:
                ...
                if isinstance(data, list) and data:
                    ...
                    assert isinstance(
                        first_item, dict
                    ), f"List items should be objects in {endpoint}"
                    assert (
                        "id" in first_item
                    ), f"Items should have ID field in {endpoint}"
    ...
    def test_cors_headers_for_frontend(self, test_client):
        ...
        for endpoint in cors_test_endpoints:
            ...
            assert response.status_code in [
                ...
                204,
                405,
            ], f"CORS preflight failed for {endpoint}"
            ...
            if get_response.status_code < 500:
                ...
                has_cors = any(
                    header in headers for header in cors_indicators
                )
                # CORS not strictly required for same-origin, but check headers are reasonable
                assert (
                    len(headers) > 0
                ), f"No response headers for {endpoint}"
...
class TestDataConsistencyContracts:
    ...
    def test_user_data_consistency(self, test_client):
        ...
        if auth_response.status_code != 200:
            pytest.skip(
                "Authentication not available for consistency test"
            )
        ...
        if profile_response.status_code == 200:
            ...
            assert (
                "id" in profile_data or "user_id" in profile_data
            ), "User profile missing ID"
    ...
    def test_conversation_message_consistency(self, test_client):
        ...
        if create_response.status_code not in [200, 201]:
            pytest.skip(
                "Cannot create conversation for consistency test"
            )

        conv_data = create_response.json()
        conv_id = conv_data.get("id") or conv_data.get(
            "conversation_id"
        )
        ...
        if message_response.status_code in [200, 201]:
            ...
            if messages_response.status_code == 200:
                ...
                test_messages = [
                    m
                    for m in messages
                    if m.get("content") == "Test message"
                ]
                assert (
                    len(test_messages) > 0
                ), "Test message not found in conversation"
