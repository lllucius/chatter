    ...
    async def test_backward_compatibility(
        ...
        try:
            ...
            for endpoint in api_endpoints:
                headers = (
                    auth_headers if endpoint["requires_auth"] else {}
                )

                if endpoint["method"] == "GET":
                    response = await client.get(
                        endpoint["path"], headers=headers
                    )
                else:
                ...
                if response.status_code == 404:
                    if "/api/v1/" in endpoint["path"]:
                        result["issues"].append(
                            "V1 API endpoint not found"
                        )
                    else:
                        result["issues"].append(
                            "Core endpoint not found"
                        )
                ...
                if endpoint["requires_auth"] and not auth_headers:
                    ...
                    if response.status_code == 401:
                        result["issues"].append(
                            "Valid authentication rejected"
                        )

                # Response should be JSON for API endpoints
                if (
                    response.status_code == 200
                    and "/api/" in endpoint["path"]
                ):
                    try:
                        response.json()
                    except Exception:
            ...
            compatible_count = sum(
                1 for r in compatibility_results if r["compatible"]
            )
            total_count = len(compatibility_results)

            print(
                f"Backward compatibility: {compatible_count}/{total_count} endpoints compatible"
            )
