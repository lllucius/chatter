...
class TestDatabaseModelIntegrity:
    ...
    def test_user_model_has_required_constraints(self):
        ...
        for constraint in expected_constraints:
            assert (
                constraint in constraint_names
    ...
    def test_foreign_key_references_are_valid(self):
        ...
        user_fks = [
            fk.target_fullname for fk in User.__table__.foreign_keys
        ]
    ...
    def test_model_indexes_are_properly_defined(self):
        ...
        [idx.name for idx in User.__table__.indexes]
    ...
    def test_model_enum_fields_are_properly_configured(self):
        ...
        assert (
            len(ConversationStatus) >= 3
        )  # Should have active, archived, deleted
        assert (
            len(MessageRole) >= 4
        )  # Should have user, assistant, system, tool
        assert (
            len(DocumentStatus) >= 4
        )  # Should have pending, processing, processed, failed
        assert (
            len(DocumentType) >= 5
    ...
    def test_prompt_model_has_required_constraints(self):
        ...
        constraint_names = [
            constraint.name
            for constraint in Prompt.__table__.constraints
        ]
    ...
    def test_provider_model_has_required_constraints(self):
        ...
        constraint_names = [
            constraint.name
            for constraint in Provider.__table__.constraints
        ]
    ...
    def test_model_def_has_required_constraints(self):
        ...
        constraint_names = [
            constraint.name
            for constraint in ModelDef.__table__.constraints
        ]
    ...
    def test_embedding_space_has_required_constraints(self):
        ...
        constraint_names = [
            constraint.name
            for constraint in EmbeddingSpace.__table__.constraints
        ]
    ...
    def test_analytics_models_have_required_constraints(self):
        ...
        conversation_stats_constraints = [
            constraint.name
        ...
        for constraint in expected_conversation_stats:
            assert (
                constraint in conversation_stats_constraints
            ), f"Missing ConversationStats constraint: {constraint}"

        # Test DocumentStats constraints
        document_stats_constraints = [
            constraint.name
        ...
        for constraint in expected_document_stats:
            assert (
                constraint in document_stats_constraints
            ), f"Missing DocumentStats constraint: {constraint}"

        # Test PromptStats constraints
        prompt_stats_constraints = [
            constraint.name
        ...
        for constraint in expected_prompt_stats:
            assert (
                constraint in prompt_stats_constraints
            ), f"Missing PromptStats constraint: {constraint}"
        ...
        for constraint in expected_profile_stats:
            assert (
                constraint in profile_stats_constraints
            ), f"Missing ProfileStats constraint: {constraint}"
    ...
    def test_toolserver_models_have_required_constraints(self):
        ...
        for constraint in expected_toolserver:
            assert (
                constraint in toolserver_constraints
            ), f"Missing ToolServer constraint: {constraint}"

        # Test ServerTool constraints
        servertool_constraints = [
            constraint.name
        ...
        for constraint in expected_servertool:
            assert (
                constraint in servertool_constraints
            ), f"Missing ServerTool constraint: {constraint}"

        # Test ToolUsage constraints
        toolusage_constraints = [
            constraint.name
        ...
        for constraint in expected_toolusage:
            assert (
                constraint in toolusage_constraints
            ), f"Missing ToolUsage constraint: {constraint}"

        # Test ToolPermission constraints
        toolpermission_constraints = [
            constraint.name
        ...
        for constraint in expected_toolpermission:
            assert (
                constraint in toolpermission_constraints
            ), f"Missing ToolPermission constraint: {constraint}"
