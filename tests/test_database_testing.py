...
class TestDatabaseTesting:
    ...
    async def test_migration_script_validation(self, db_session: AsyncSession):
        ...
        try:
            ...
            expected_tables = [
                'users', 'chats', 'messages', 'documents', 'providers', 'models',
                'embedding_spaces', 'agents', 'profiles', 'jobs', 'events'
            ]
            
            existing_core_tables = [table for table in expected_tables if table in existing_tables]
            ...
            if existing_core_tables:
                ...
                for table_name in existing_core_tables:
                    ...
                    if 'id' not in column_names:
                        print(f"Warning: Table {table_name} missing 'id' column")
                    
                    # Check for created_at/updated_at timestamps
                    timestamp_cols = [col for col in column_names if col in ['created_at', 'updated_at']]
                    if not timestamp_cols:
                        print(f"Info: Table {table_name} missing timestamp columns")
            else:
                pytest.skip("No core tables found - database may need migration")
    ...
    @pytest.mark.database
    async def test_connection_pooling_behavior(self, db_session: AsyncSession):
        ...
        try:
            ...
            valid_ids = [cid for cid in connection_ids if cid is not None]
            assert len(valid_ids) > 0, "No valid connections established"
            
            # Test connection reuse (some IDs should be repeated with pooling)
            unique_ids = set(valid_ids)
            if len(unique_ids) < len(valid_ids):
                print(f"Connection pooling active: {len(valid_ids)} requests, {len(unique_ids)} unique connections")
            else:
                print("Connection pooling may not be active or pool size is large")
    ...
    @pytest.mark.database 
    async def test_transaction_rollback_testing(self, db_session: AsyncSession):
        ...
        try:
            ...
            try:
                ...
                assert count_before_rollback == 1, "Test data not inserted"
    ...
    @pytest.mark.database
    async def test_index_performance_validation(self, db_session: AsyncSession):
        ...
        try:
            ...
            if not existing_tables:
                pytest.skip("No tables found for index performance testing")
            
            # Test index usage with EXPLAIN
            for table_name in existing_tables[:3]:  # Test first 3 tables
                try:
                    # Get table columns
                    columns = inspector.get_columns(table_name)
                    column_names = [col['name'] for col in columns]
                    ...
                    if 'id' in column_names:
                        ...
                        explain_result = await db_session.execute(text(f"EXPLAIN {query}"))
                        ...
                        if "Index Scan" in explain_text or "Seq Scan" in explain_text:
                            print(f"Table {table_name}: Query plan available")
                        
                    # Test for timestamp columns if they exist
                    timestamp_cols = [col for col in column_names if 'created_at' in col or 'updated_at' in col]
                    if timestamp_cols:
                        ...
                        try:
                            ...
                            print(f"Table {table_name}: Timestamp query plan available")
                        except Exception:
    ...
    @pytest.mark.database
    async def test_foreign_key_constraint_enforcement(self, db_session: AsyncSession):
        ...
        try:
            ...
            for table_name in existing_tables:
                try:
                    ...
                    if foreign_keys:
                        ...
                        print(f"Table {table_name} has {len(foreign_keys)} foreign key constraints")
                        ...
                        for fk in foreign_keys:
                            ...
                            if local_columns and referenced_table in existing_tables:
                                ...
                                try:
                                    ...
                                    print(f"Foreign key constraint exists: {table_name} -> {referenced_table}")
            ...
            if foreign_keys_found:
                ...
                print("Foreign key constraints: NONE FOUND (may be expected)")
    ...
    async def test_data_integrity_and_consistency(self, db_session: AsyncSession):
        ...
        try:
            ...
            for table_name in existing_tables[:5]:  # Test first 5 tables
                try:
                    # Count total records
                    count_result = await db_session.execute(text(f"SELECT COUNT(*) FROM {table_name}"))
                    total_count = count_result.scalar()
                    
                    if total_count > 0:
                        # Test for NULL values in non-nullable columns
                        columns = inspector.get_columns(table_name)
                        non_nullable_cols = [col['name'] for col in columns if not col['nullable']]
                        ...
                        for col in non_nullable_cols:
                            ...
                            assert null_count == 0, f"Found NULL values in non-nullable column {table_name}.{col}"
                        
                        print(f"Table {table_name}: {total_count} records, integrity verified")
                    else:
    ...
    async def test_bulk_operation_performance(self, db_session: AsyncSession):
        ...
        try:
            ...
            count_result = await db_session.execute(text("SELECT COUNT(*) FROM bulk_test"))
            inserted_count = count_result.scalar()
            
            assert inserted_count == total_records, f"Expected {total_records}, got {inserted_count}"
            
            # Test bulk select performance
            select_start = time.time()
            result = await db_session.execute(text("SELECT * FROM bulk_test"))
            ...
            assert insert_duration < 10.0, f"Bulk insert too slow: {insert_duration:.2f}s"
            assert select_duration < 5.0, f"Bulk select too slow: {select_duration:.2f}s"
            assert len(all_records) == total_records, "Bulk select returned wrong count"
            
            print(f"Bulk operations: Insert {total_records} records in {insert_duration:.2f}s, Select in {select_duration:.2f}s")
            
            # Test bulk update performance
            update_start = time.time()
            await db_session.execute(text("UPDATE bulk_test SET data = data || ' (updated)'"))
            await db_session.commit()
            update_end = time.time()
            
            update_duration = update_end - update_start
            assert update_duration < 10.0, f"Bulk update too slow: {update_duration:.2f}s"
            
            print(f"Bulk update: Updated {total_records} records in {update_duration:.2f}s")
            
        except Exception as e:
            pytest.skip(f"Bulk operation performance test skipped: {e}")
    
    @pytest.mark.database
    async def test_connection_security_and_encryption(self, db_session: AsyncSession):
        ...
        try:
            ...
            try:
                current_user_result = await db_session.execute(text("SELECT current_user"))
    ...
    @pytest.mark.database
    async def test_database_backup_and_recovery_readiness(self, db_session: AsyncSession):
        """Test database backup and recovery readiness."""
        try:
            # Test if database supports point-in-time recovery
            try:
                wal_result = await db_session.execute(text("SHOW wal_level"))
                ...
                if wal_level in ['replica', 'logical']:
                    ...
                    print("Database may not be optimally configured for backup")
            ...
            try:
                ...
                await db_session.execute(text("INSERT INTO backup_test DEFAULT VALUES"))
                await db_session.commit()
                
                # Verify the record exists
                result = await db_session.execute(text("SELECT COUNT(*) FROM backup_test"))
                count = result.scalar()
    ...
    @pytest.mark.database
    async def test_query_performance_optimization(self, db_session: AsyncSession):
        ...
        try:
            ...
            for test in performance_tests:
                ...
                assert query_time < test["max_time"], f"{test['name']} query too slow: {query_time:.3f}s"
            ...
            if existing_tables:
                ...
                start_time = time.time()
                count_result = await db_session.execute(text(f"SELECT COUNT(*) FROM {test_table}"))
                count_time = time.time() - start_time
                
                record_count = count_result.scalar()
                
                # Performance should scale reasonably with data size
                max_count_time = 1.0 + (record_count / 10000)  # 1s base + 1s per 10k records
                assert count_time < max_count_time, f"COUNT query too slow: {count_time:.3f}s for {record_count} records"
                
                print(f"COUNT query on {test_table}: {count_time:.3f}s ({record_count} records)")
    ...
    async def test_database_api_integration_consistency(self, client: AsyncClient, auth_headers: Dict[str, str], db_session: AsyncSession):
        """Test consistency between database state and API responses."""
        if not auth_headers:
            pytest.skip("Authentication required for database-API integration test")
                ...
                if user_id:
                    ...
                    try:
                        ...
                        if user_db_data:
                            ...
                            assert str(user_db_data[0]) == str(user_id), "User ID mismatch"
                            assert user_db_data[1] == profile_data.get("username"), "Username mismatch"
                            assert user_db_data[2] == profile_data.get("email"), "Email mismatch"
                            
                            print("User data consistency: VERIFIED")
                        
                    except Exception:
                        # Users table might not exist or have different structure
                        print("User data consistency: Could not verify (table structure unknown)")
            ...
            if chats_response.status_code == 200:
                ...
                try:
                    ...
                    chat_count_result = await db_session.execute(
                        text("SELECT COUNT(*) FROM chats WHERE user_id = :user_id"),
                        {"user_id": user_id}
                    )
                    db_chat_count = chat_count_result.scalar()
                    
                    # API response should match database count
                    api_chat_count = len(api_chats) if isinstance(api_chats, list) else api_chats.get("total", 0)
                    
                    if db_chat_count is not None:
                        assert api_chat_count == db_chat_count, f"Chat count mismatch: API={api_chat_count}, DB={db_chat_count}"
                        print(f"Chat data consistency: VERIFIED ({db_chat_count} chats)")
                    
                except Exception:
                    # Chats table might not exist or have different structure
                    print("Chat data consistency: Could not verify (table structure unknown)")
