...
class TestDocumentsUnit:
    ...
    @pytest.mark.unit
    async def test_upload_document_endpoint_exists(
    ...
    @pytest.mark.unit
    async def test_list_documents_endpoint_exists(
        self, client: AsyncClient, auth_headers: dict
    ):
    ...
    @pytest.mark.unit
    async def test_search_documents_endpoint_exists(
        self, client: AsyncClient, auth_headers: dict
    ):
        """Test that search documents endpoint exists."""
        # Test without auth
        response = await client.post(
            "/api/v1/documents/search", json={}
        )
        assert response.status_code == 401

        # Test with auth but invalid data
        response = await client.post(
            "/api/v1/documents/search", headers=auth_headers, json={}
        )
        assert response.status_code == 422  # Should require query field

    @pytest.mark.unit
    async def test_get_document_endpoint_exists(
        self, client: AsyncClient, auth_headers: dict
    ):
    ...
    @pytest.mark.unit
    async def test_delete_document_endpoint_exists(
        self, client: AsyncClient, auth_headers: dict
    ):
    ...
    @pytest.mark.unit
    async def test_update_document_endpoint_exists(
        self, client: AsyncClient, auth_headers: dict
    ):
        """Test that update document endpoint exists."""
        # Test without auth
        response = await client.put(
            "/api/v1/documents/nonexistent-id", json={}
        )
        assert response.status_code == 401
    ...
    @pytest.mark.unit
    async def test_get_document_chunks_endpoint_exists(
        self, client: AsyncClient, auth_headers: dict
    ):
        """Test that get document chunks endpoint exists."""
        # Test without auth
        response = await client.get(
            "/api/v1/documents/nonexistent-id/chunks"
        )
        assert response.status_code == 401
    ...
    @pytest.mark.unit
    async def test_process_document_endpoint_exists(
        self, client: AsyncClient, auth_headers: dict
    ):
        """Test that process document endpoint exists."""
        # Test without auth
        response = await client.post(
            "/api/v1/documents/nonexistent-id/process", json={}
        )
        assert response.status_code == 401

        # Test with auth - should get 404 for nonexistent document
        response = await client.post(
            "/api/v1/documents/nonexistent-id/process",
            headers=auth_headers,
            json={},
        )
        assert response.status_code == 404
    ...
    @pytest.mark.unit
    async def test_get_document_stats_endpoint_exists(
        self, client: AsyncClient, auth_headers: dict
    ):
    ...
    @pytest.mark.unit
    async def test_download_document_endpoint_exists(
        self, client: AsyncClient, auth_headers: dict
    ):
        """Test that download document endpoint exists."""
        # Test without auth
        response = await client.get(
            "/api/v1/documents/nonexistent-id/download"
        )
        assert response.status_code == 401
