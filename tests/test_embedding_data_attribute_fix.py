...
class TestSafeOpenAIEmbeddingsDataFix:
    ...
    async def test_safe_embeddings_handles_data_attribute_error(self):
        ...
        class MockSafeOpenAIEmbeddings:
            ...
            async def aembed_documents(
                self, texts, chunk_size=None, **kwargs
            ):
                ...
                try:
                    ...
                    if (
                        ...
                        return (
                            await self._safe_embed_documents_fallback(
    ...
    @pytest.mark.asyncio
    async def test_safe_embeddings_handles_problematic_data_object(
        self,
    ):
    ...
    def test_error_pattern_detection(self):
        ...
        for error_msg in error_messages:
            ...
            assert (
                should_catch
            ), f"Error '{error_msg}' should be caught by our patterns"
        ...
        for error_msg in unrelated_errors:
            ...
            assert (
                should_not_catch
            ), f"Error '{error_msg}' should NOT be caught by our patterns"
...
if __name__ == "__main__":
    ...
    async def run_async_tests():
        ...
        print(
            "Running test_safe_embeddings_handles_data_attribute_error..."
        )
        await test_instance.test_safe_embeddings_handles_data_attribute_error()
        print("✓ PASSED")

        print(
            "Running test_safe_embeddings_handles_problematic_data_object..."
        )
        try:
            await test_instance.test_safe_embeddings_handles_problematic_data_object()
            print(
                "✓ PASSED - safely handled problematic response (did not crash)"
            )
        except ValueError as e:
            if "Unhandled response format" in str(e):
                print(
                    "✓ PASSED - correctly raised ValueError for unhandled format"
                )
            else:
            ...
            if "'list' object has no attribute 'data'" in str(e):
                print(
                    "❌ FAILED - the fix didn't prevent the AttributeError"
                )
            else:
