...
from chatter.services.embeddings import SafeOpenAIEmbeddings, EmbeddingService
...
class TestEmbeddingResponseFormatFix:
    ...
    @pytest.mark.asyncio
    async def test_safe_embeddings_handles_problematic_response_format(self):
        ...
        assert all(all(isinstance(val, (int, float)) for val in emb) for emb in result)
        
        # Test single query
        query_result = await embeddings.aembed_query("test query")
        assert isinstance(query_result, list)
        assert len(query_result) == 3
        assert all(isinstance(val, (int, float)) for val in query_result)
    ...
    @pytest.mark.asyncio
    async def test_embedding_service_creates_safe_openai_embeddings(self):
