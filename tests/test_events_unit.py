...
class TestEventsUnit:
    ...
    @pytest.mark.unit
    async def test_admin_stream_requires_auth(
        self, client: AsyncClient
    ):
    ...
    @pytest.mark.unit
    async def test_events_stats_requires_auth(
        self, client: AsyncClient
    ):
    ...
    @pytest.mark.unit
    async def test_broadcast_test_requires_auth(
        self, client: AsyncClient
    ):
    ...
    async def test_events_stats_success(
        ...
        mock_sse_service.get_stats.return_value = {
            ...
            "connections_by_user": {"user1": 2, "user2": 3},
        }

        response = await client.get(
            "/api/v1/events/stats", headers=auth_headers
        )
    ...
    @pytest.mark.unit
    async def test_test_event_invalid_data(
    ...
    @pytest.mark.unit
    async def test_broadcast_test_invalid_data(
        self, client: AsyncClient, auth_headers: dict
    ):
    ...
    async def test_events_stream_rate_limit(
        ...
        response = await client.get(
            "/api/v1/events/stream", headers=auth_headers
        )
    ...
    async def test_events_stream_connection_limit(
        ...
        mock_sse_service.create_connection.side_effect = ValueError(
    ...
    async def test_admin_stream_requires_admin(
        ...
        response = await client.get(
            "/api/v1/events/admin/stream", headers=auth_headers
        )
    ...
    async def test_events_stats_sse_service_error(
        ...
        mock_sse_service.get_stats.side_effect = Exception(
            "SSE service error"
        )
    ...
    async def test_test_event_sse_service_error(
        ...
        mock_sse_service.broadcast_to_user.side_effect = Exception(
            "SSE service error"
        )
    ...
    async def test_broadcast_test_sse_service_error(
        ...
        mock_sse_service.broadcast.side_effect = Exception(
            "SSE service error"
        )
