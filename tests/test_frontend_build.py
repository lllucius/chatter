...
def test_sdk_build():
    ...
    assert dist_path.exists(), f"Dist directory not created at {dist_path}"
    
    # Check if main files exist
    assert (dist_path / "index.js").exists(), "index.js not found in dist"
    assert (dist_path / "index.d.ts").exists(), "index.d.ts not found in dist"
...
def test_frontend_dependency_resolution():
    ...
    assert result.returncode == 0, f"chatter-sdk dependency not resolved: {result.stderr}"
    assert "chatter-sdk@0.1.0" in result.stdout, f"Unexpected chatter-sdk version: {result.stdout}"