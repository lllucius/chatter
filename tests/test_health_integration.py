...
class TestHealthIntegration:
    ...
    @pytest.mark.integration
    async def test_readiness_check_database_timeout(self, client: AsyncClient):
        ...
        with patch('chatter.api.health.health_check') as mock_health_check:
            mock_health_check.side_effect = asyncio.TimeoutError("Database timeout")
    ...
    @pytest.mark.integration
    async def test_readiness_check_database_error(self, client: AsyncClient):
        ...
        with patch('chatter.api.health.health_check') as mock_health_check:
            ...
            assert data["checks"]["database"]["status"] == "unhealthy"
            assert data["checks"]["database"]["connected"] is False
            assert "Connection failed" in data["checks"]["database"]["error"]

    @pytest.mark.integration
    async def test_health_endpoints_response_consistency(self, client: AsyncClient):
    ...
    @pytest.mark.integration
    async def test_correlation_trace_nonexistent_id(self, client: AsyncClient):
    ...
    @pytest.mark.integration
    async def test_metrics_endpoint_structure(self, client: AsyncClient):
        ...
        # Required fields
        required_fields = ["timestamp", "service", "version", "environment", "health", "performance", "endpoints"]
        for field in required_fields:
            assert field in data, f"Required field '{field}' missing from metrics response"
    ...
    @pytest.mark.integration
    async def test_multiple_readiness_checks_isolation(self, client: AsyncClient):
    ...
    @pytest.mark.integration
    async def test_health_endpoints_concurrent_access(self, client: AsyncClient):
        ...
        tasks = [
            ...
            asyncio.create_task(client.get("/trace/concurrent-test-123"))
        ]
        ...
        for i, response in enumerate(responses):
            data = response.json()
            assert "service" in data or "correlation_id" in data  # All should have one of these