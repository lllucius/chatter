...
class TestIntegrationWorkflows:
    ...
    async def test_document_to_chat_workflow(
        self, client: AsyncClient, auth_headers: dict[str, str]
    ):
        """Test complete document-to-chat integration workflow."""
        if not auth_headers:
            pytest.skip(
                "Authentication required for document-to-chat workflow test"
            )
        ...
        try:
            ...
            if upload_response.status_code not in [200, 201]:
                pytest.skip(
                    "Document upload failed - cannot test workflow"
                )
            ...
            chat_response = await client.post(
                "/api/v1/chats", json=chat_data, headers=auth_headers
            )
            ...
            if chat_response.status_code not in [200, 201]:
                pytest.skip(
                    "Chat creation failed - cannot test workflow"
                )
            ...
            assert message_response.status_code in [
                200,
                201,
            ], "Message sending failed in workflow"
            ...
            chat_messages_response = await client.get(
                f"/api/v1/chats/{chat_id}/messages",
                headers=auth_headers,
            )
            ...
            if chat_messages_response.status_code == 200:
                ...
                assert (
                    len(messages) >= 1
                ), "Chat should contain the sent message"
                ...
                assert (
                    len(user_messages) >= 1
                ), "Chat should contain user message"

            # Step 7: Verify document is still accessible
            doc_check_response = await client.get(
    ...
    @pytest.mark.integration
    async def test_user_agent_interaction_workflows(
        self, client: AsyncClient, auth_headers: dict[str, str]
    ):
        """Test user-agent interaction workflows."""
        if not auth_headers:
            pytest.skip(
                "Authentication required for user-agent workflow test"
            )
        ...
        try:
            ...
            if agents_response.status_code == 200:
                ...
                if isinstance(agents, list) and agents:
                    ...
                    agent_id = agent["id"]
                    print(
                        f"Using existing agent: {agent.get('name', agent_id)}"
                    )
                else:
                    ...
                    if create_agent_response.status_code in [200, 201]:
                        ...
                        agent_id = agent["id"]
                        print(
                            f"Created new agent: {agent.get('name', agent_id)}"
                        )
                    else:
                        pytest.skip(
                            "Cannot create agent for workflow test"
                        )
            else:
            ...
            if chat_response.status_code not in [200, 201]:
                ...
                chat_response = await client.post(
                    "/api/v1/chats",
                    json=chat_data_fallback,
                    headers=auth_headers,
                )

            if chat_response.status_code not in [200, 201]:
                pytest.skip(
                    "Cannot create chat for agent workflow test"
                )
            ...
            for user_message in user_messages:
                ...
                if message_response.status_code in [200, 201]:
                    ...
                    print(
                        f"Message sending failed: {message_response.status_code}"
                    )
            ...
            if history_response.status_code == 200:
                ...
                assert user_messages_count >= len(
                    user_messages
                ), "Not all user messages were recorded"

                if agent_messages_count > 0:
                    print(
                        f"Agent interaction successful: {user_messages_count} user messages, {agent_messages_count} agent responses"
                    )
                else:
                    print(
                        f"Agent interaction recorded: {user_messages_count} user messages (no agent responses detected)"
                    )
            ...
            pytest.skip(
                f"User-agent interaction workflow test skipped: {e}"
            )

    @pytest.mark.integration
    async def test_analytics_across_multiple_services(
        self, client: AsyncClient, auth_headers: dict[str, str]
    ):
        """Test analytics integration across multiple services."""
        if not auth_headers:
            pytest.skip(
                "Authentication required for analytics workflow test"
            )
        ...
        try:
            ...
            for i in range(3):
                ...
                files = {
                    "file": (
                        f"analytics_doc_{i+1}.txt",
                        test_file,
                        "text/plain",
                    )
                }
                data = {
                    "title": f"Analytics Document {i+1}",
                    "description": f"Document {i+1} for analytics testing",
                }
                ...
                if doc_response.status_code in [200, 201]:
                    activities_performed.append(
                        f"document_upload_{i+1}"
                    )

            # Activity 2: Create chats
            for i in range(2):
                chat_data = {
                    "title": f"Analytics Chat {i+1}",
                    "description": f"Chat {i+1} for analytics testing",
                }
                chat_response = await client.post(
                    "/api/v1/chats",
                    json=chat_data,
            ...
            for endpoint in endpoint_interactions:
                ...
                if response.status_code == 200:
                    activities_performed.append(
                        f"endpoint_access_{endpoint.split('/')[-1]}"
                    )

            print(
                f"Generated {len(activities_performed)} activities for analytics"
            )
            ...
            for endpoint in analytics_endpoints:
                ...
                if response.status_code == 200:
                    ...
                    if isinstance(analytics_data, dict):
                        ...
                        if found_metrics:
                            print(
                                f"Analytics endpoint {endpoint}: {len(found_metrics)} relevant metrics found"
                            )
                elif response.status_code == 404:
                    continue  # Endpoint not available
                else:
                    print(
                        f"Analytics endpoint {endpoint}: Status {response.status_code}"
                    )

            if analytics_available:
                print(
                    f"Analytics integration: {len(analytics_available)} endpoints available"
                )
                ...
                if len(analytics_available) > 1:
                    ...
                    if len(user_counts) > 1:
                        ...
                        if consistent_counts:
                            print(
                                "Analytics consistency: User counts consistent across endpoints"
                            )
                        else:
                            print(
                                f"Analytics consistency: User counts inconsistent: {user_counts}"
                            )
            else:
                print(
                    "Analytics integration: No analytics endpoints available"
                )
            ...
            pytest.skip(
                f"Analytics integration workflow test skipped: {e}"
            )
    ...
    async def test_external_service_integration(
        ...
        try:
                ...
                if response.status_code == 200:
                    ...
                    if isinstance(service_data, list) and service_data:
                        ...
                        if "providers" in endpoint:
                            ...
                            assert (
                                "name" in first_item
                            ), "Provider should have name"

                            if "active" in first_item:
                                print(
                                    f"Provider {first_item['name']}: {'active' if first_item['active'] else 'inactive'}"
                                )
                            ...
                            if "provider_id" in first_item:
                                print(
                                    f"Model {first_item['name']}: Provider {first_item['provider_id']}"
                                )

                        elif "embedding" in endpoint:
                            assert (
                                "id" in first_item
                            ), "Embedding space should have ID"

                            if "dimensions" in first_item:
                                print(
                                    f"Embedding space: {first_item['dimensions']} dimensions"
                                )
            ...
            if external_services:
                print(
                    f"External services configured: {len(external_services)} types"
                )
                ...
                for endpoint in status_endpoints:
                    ...
                    if response.status_code == 200:
                        ...
                        print(
                            f"External service status from {endpoint}: Available"
                        )
                ...
                print(
                    "External service integration: No external services configured"
                )
            ...
            pytest.skip(
                f"External service integration test skipped: {e}"
            )
        ...
        try:
            ...
            for endpoint in cache_test_endpoints:
                ...
                cache_result = {
                    ...
                    "times": times,
                    "avg_time": avg_time,
                    "potential_caching": second_time < first_time
                    or third_time < avg_time,
                    "response_consistent": (
                        first_response.text
                ...
                if cache_result["potential_caching"]:
                    print(
                        f"Cache behavior detected for {endpoint}: times {[f'{t:.3f}' for t in times]}s"
                    )
                else:
                    print(
                        f"No clear cache pattern for {endpoint}: times {[f'{t:.3f}' for t in times]}s"
                    )

            # Test cache-related headers
            for endpoint in cache_test_endpoints[
                :2
            ]:  # Test first 2 endpoints
                headers = auth_headers if "/api/v1/" in endpoint else {}
                ...
                if response.status_code == 200:
                    ...
                    if found_cache_headers:
                        print(
                            f"Cache headers in {endpoint}: {found_cache_headers}"
                        )
                ...
                if create_response.status_code in [200, 201]:
                    ...
                    if updated_list_response.status_code == 200:
                        ...
                        if isinstance(updated_chats, list):
                            ...
                            if new_chat_found:
                                print(
                                    "Cache invalidation: Working (new data appears immediately)"
                                )
                            else:
                                print(
                                    "Cache invalidation: May not be working (new data not visible)"
                                )
    ...
    async def test_error_propagation_and_recovery(
        ...
        try:
            ...
            invalid_file_data = (
                b"This is not a valid file content that might cause processing errors"
                * 1000
            )
            files = {
                "file": (
                    "invalid_large_file.txt",
                    invalid_file_data,
                    "text/plain",
                )
            }
            data = {
                "title": "Invalid Large File",
                "description": "Testing error handling",
            }

            if auth_headers:
                error_upload_response = await client.post(
                    "/api/v1/documents/upload",
                    files=files,
                    data=data,
                    headers=auth_headers,
                ...
                error_scenarios.append(
                    {
                        ...
                        "handled_gracefully": error_upload_response.status_code
                        in [400, 413, 422],
                        "error_message": (
                            error_upload_response.text[:200]
                            if error_upload_response.status_code >= 400
                            else None
                        ),
                    }
                )
            ...
            if auth_headers:
                ...
                invalid_chat_response = await client.post(
                    "/api/v1/chats",
                ...
                error_scenarios.append(
                    {
                        ...
                        "handled_gracefully": invalid_chat_response.status_code
                        in [400, 422],
                        "error_message": (
                            invalid_chat_response.text[:200]
                            if invalid_chat_response.status_code >= 400
                            else None
                        ),
                    }
                )
                ...
                error_scenarios.append(
                    {
                        ...
                        "handled_gracefully": response.status_code
                        in [404, 400],
                        "error_message": (
                            response.text[:200]
            ...
            graceful_errors = [
                s for s in error_scenarios if s["handled_gracefully"]
            ]
            total_errors = len(error_scenarios)

            if total_errors > 0:
                graceful_percentage = (
                    len(graceful_errors) / total_errors
                )
                print(
                    f"Error handling: {len(graceful_errors)}/{total_errors} ({graceful_percentage:.1%}) handled gracefully"
                )
                ...
                for scenario in error_scenarios:
                    if (
                        ...
                        if leaks_info:
                            print(
                                f"  Warning: {scenario['scenario']} may leak sensitive information"
                            )
                        ...
                        if is_friendly:
                            print(
                                f"  Good: {scenario['scenario']} has user-friendly error message"
                            )
            ...
            if auth_headers:
                ...
                recovery_tests.append(
                    {
                        ...
                        "successful": recovery_response.status_code
                        in [200, 201],
                ...
                profile_recovery_response = await client.get(
                    "/api/v1/auth/profile", headers=auth_headers
                )
                recovery_tests.append(
                    {
                        "operation": "profile_access_after_errors",
                        "successful": profile_recovery_response.status_code
                        == 200,
            ...
            if recovery_tests:
                successful_recoveries = [
                    t for t in recovery_tests if t["successful"]
                ]
                print(
                    f"System recovery: {len(successful_recoveries)}/{len(recovery_tests)} operations successful after errors"
                )
            ...
            pytest.skip(
                f"Error propagation and recovery test skipped: {e}"
            )

    @pytest.mark.integration
    @pytest.mark.slow
    async def test_service_dependencies_and_graceful_degradation(
        self, client: AsyncClient, auth_headers: dict[str, str]
    ):
        ...
        try:
            ...
            for service in core_services:
                ...
                try:
                    response = await client.get(
                        service["endpoint"], headers=headers
                    )
            ...
            print(
                f"Service availability: {len(available_services)}/{len(core_services)} services available"
            )
            ...
            if unavailable_services:
                print(
                    f"  Unavailable: {', '.join(unavailable_services)}"
                )
            ...
            if service_status.get("authentication", {}).get(
                "available"
            ) and service_status.get("database", {}).get("available"):
                    ...
                    degradation_scenarios.append(
                        {
                            ...
                            "working": profile_response.status_code
                            == 200,
            ...
            if degradation_scenarios:
                graceful_count = sum(
                    ...
                    if s.get("degraded_gracefully", True)
                )
                working_count = sum(
                    1
                    for s in degradation_scenarios
                    if s.get("working", False)
                )

                print(
                    f"Graceful degradation: {graceful_count}/{len(degradation_scenarios)} scenarios handle failures gracefully"
                )
                print(
                    f"Optional services: {working_count}/{len(degradation_scenarios)} working"
                )
            ...
            for endpoint in health_endpoints:
                ...
                if response.status_code == 200:
                    ...
                    if isinstance(health_data, dict):
                        ...
                        if dependency_info:
                            print(
                                f"Health monitoring provides dependency info: {dependency_info}"
                            )
            ...
            if not health_available:
                print(
                    "Health monitoring: No comprehensive health endpoints found"
                )

            print(
                "Service dependencies and graceful degradation: TESTED"
            )
