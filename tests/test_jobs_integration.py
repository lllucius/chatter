...
class TestJobsIntegration:
    ...
    async def test_job_complete_lifecycle(self, client: AsyncClient, auth_headers: dict, db_session: AsyncSession):
        ...
        stats_response = await client.get("/api/v1/jobs/stats/overview", headers=auth_headers)
    ...
    async def test_job_listing_and_filtering(self, client: AsyncClient, auth_headers: dict, db_session: AsyncSession):
        ...
        # Test pagination
        page_response = await client.get("/api/v1/jobs/?page=1&per_page=2", headers=auth_headers)
        assert page_response.status_code == 200
        page_data = page_response.json()
        assert len(page_data["jobs"]) <= 2
        
        # Test status filtering (if any jobs have completed)
        status_response = await client.get("/api/v1/jobs/?status=pending", headers=auth_headers)
    ...
    async def test_job_cancellation_workflow(self, client: AsyncClient, auth_headers: dict, db_session: AsyncSession):
        ...
        cancel_response = await client.post(f"/api/v1/jobs/{job_id}/cancel", headers=auth_headers)
    ...
    async def test_job_error_scenarios(self, client: AsyncClient, auth_headers: dict, db_session: AsyncSession):
        ...
        cancel_response = await client.post(f"/api/v1/jobs/{nonexistent_id}/cancel", headers=auth_headers)
        ...
        for invalid_id in invalid_ids:
            if invalid_id:  # Skip empty string for URL path
                get_response = await client.get(f"/api/v1/jobs/{invalid_id}", headers=auth_headers)
                assert get_response.status_code == 400
                
                cancel_response = await client.post(f"/api/v1/jobs/{invalid_id}/cancel", headers=auth_headers)
    ...
    async def test_job_validation_scenarios(self, client: AsyncClient, auth_headers: dict, db_session: AsyncSession):
        ...
        assert response.status_code == 400  # Bad request for invalid function
        ...
        response = await client.post("/api/v1/jobs/", json=incomplete_data, headers=auth_headers)
        ...
        response = await client.post("/api/v1/jobs/", json=invalid_priority_data, headers=auth_headers)
        assert response.status_code == 422  # Validation error
    ...
    async def test_job_stats_accuracy(self, client: AsyncClient, auth_headers: dict, db_session: AsyncSession):
        ...
        # Get updated stats
        final_response = await client.get("/api/v1/jobs/stats/overview", headers=auth_headers)
        ...
        assert total_by_status <= final_stats["total_jobs"]  # May be equal or less due to cleanup
    ...
    async def test_job_cleanup_functionality(self, client: AsyncClient, auth_headers: dict, db_session: AsyncSession):
        ...
        # Run cleanup
        cleanup_response = await client.post("/api/v1/jobs/cleanup", headers=auth_headers)
    ...
    async def test_concurrent_job_operations(self, client: AsyncClient, auth_headers: dict, db_session: AsyncSession):
        ...
        for job_id in created_job_ids:
            operation_tasks.append(
                asyncio.create_task(client.get(f"/api/v1/jobs/{job_id}", headers=auth_headers))
            )
        
        # Also get stats and list jobs concurrently
        operation_tasks.extend([
            asyncio.create_task(client.get("/api/v1/jobs/stats/overview", headers=auth_headers)),
            asyncio.create_task(client.get("/api/v1/jobs/", headers=auth_headers))
        ])
