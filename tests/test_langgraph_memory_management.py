...
class TestLangGraphMemoryManagement:
    ...
    async def test_focus_mode_uses_only_last_message(self):
        ...
        messages = [
            ...
            AIMessage(content="I'm an AI assistant created by Anthropic."),
    ...
    async def test_conversation_summary_creation(self):
        ...
        assert "User greeted" not in summary_prompt  # Summary prompt, not the result
        ...
        response = await mock_llm.ainvoke([HumanMessage(content=summary_prompt)])
        summary = response.content
        
        assert summary == "User greeted assistant and asked about assistant's name. Assistant responded helpfully."
        assert len(summary) < len(summary_prompt)  # Summary should be more concise
    ...
    async def test_no_duplicate_history_in_context(self):
        ...
        if len(messages) > memory_window:
            ...
            # 4 recent messages
            assert len(recent_messages) == 4
            assert recent_messages[0].content == "Hello! How can I help you today?"
...
class TestMemoryManagementConfiguration:
    ...
    def test_focus_mode_parameter_exists(self):
        ...
        assert focus_mode_param.default == False  # Default should be False
