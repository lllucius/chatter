...
async def test_memory_summary_filtering():
    ...
    summary_response = AIMessage(content="There is no conversation history to summarize. This conversation just started with your first message. What would you like to talk about? I can summarize our conversation at the end if you'd like.")
    normal_response = AIMessage(content="I don't have a personal name, but I'm happy to play along. If you'd like, you can call me \"Bill\" or any other name you'd like. I'll respond accordingly.")
    
    mock_llm.ainvoke = AsyncMock(side_effect=[summary_response, normal_response])
    ...
    initial_state: ConversationState = {
        'messages': [
            ...
            HumanMessage(content="can I call you bill?")  # This should trigger memory management
        ],
        'user_id': 'test_user',
    ...
    result = await manager.run_workflow(workflow, initial_state, "test_thread")
...
async def test_summary_context_formatting():
    ...
    call_args = mock_llm.ainvoke.call_args[0][0]  # Get the messages passed to LLM
    ...
    assert "Context from previous conversation:" in summary_context or summary_context.startswith("Summary:")
