...
from chatter.schemas.chat import MessageRatingUpdate, MessageRatingResponse
...
async def test_message_service_rating_validation():
    ...
    with pytest.raises(ValidationError) as exc_info:
        await service.update_message_rating("conv1", "msg1", "user1", 6.0)
    
    assert "Rating must be between 0.0 and 5.0" in str(exc_info.value)
    
    with pytest.raises(ValidationError) as exc_info:
        await service.update_message_rating("conv1", "msg1", "user1", -1.0)
...
def test_message_response_with_rating():
    ...
    message_response_no_rating = MessageResponse(**message_data_no_rating)
