...
class TestPerformance:
    ...
    async def test_authentication_endpoint_performance(
        ...
        try:
            ...
            if registration_times:
                avg_registration_time = statistics.mean(
                    registration_times
                )
                ...
                ), f"Average registration time too slow: {avg_registration_time}s"
                assert (
                    max_registration_time < 5.0
                ), f"Max registration time too slow: {max_registration_time}s"

                print(
                    f"Registration performance: avg={avg_registration_time:.3f}s, max={max_registration_time:.3f}s"
                )
    ...
    @pytest.mark.performance
    async def test_chat_message_processing_benchmarks(
        self, client: AsyncClient, auth_headers: dict[str, str]
    ):
        """Test chat message processing performance benchmarks."""
        if not auth_headers:
            pytest.skip(
                "Authentication required for chat performance test"
            )
        ...
        try:
            ...
            chat_data = {
                ...
                "description": "Testing message performance",
            }
            ...
            if chat_response.status_code not in [200, 201]:
                pytest.skip(
                    "Could not create test chat for performance testing"
                )
            ...
            for i in range(20):
                ...
                if response.status_code == 404:
                    pytest.skip(
                        "Chat messaging endpoint not implemented yet"
                    )
            ...
            if message_times:
                ...
                # Performance assertions
                assert (
                    avg_message_time < 1.0
                ), f"Average message processing too slow: {avg_message_time}s"
                assert (
                    max_message_time < 3.0
                ), f"Max message processing too slow: {max_message_time}s"

                print(
                    f"Message processing performance: avg={avg_message_time:.3f}s, max={max_message_time:.3f}s"
                )
    ...
    @pytest.mark.performance
    async def test_document_upload_performance_validation(
        self, client: AsyncClient, auth_headers: dict[str, str]
    ):
        """Test document upload performance validation."""
        if not auth_headers:
            pytest.skip(
                "Authentication required for document performance test"
            )
        ...
        try:
            ...
            if upload_times:
                for upload in upload_times:
                    ...
                    assert (
                        upload["time"] < expected_max_time
                    ), f"Upload too slow for {upload['size']}B: {upload['time']:.3f}s"

                avg_time = statistics.mean(
                    [u["time"] for u in upload_times]
                )
                print(
                    f"Document upload performance: avg={avg_time:.3f}s"
                )

        except Exception as e:
            pytest.skip(
                f"Document upload performance test skipped: {e}"
            )
    ...
    async def test_concurrent_request_handling(
        ...
        if not auth_headers:
            pytest.skip(
                "Authentication required for concurrent request test"
            )
        ...
        try:
            ...
            if successful_results:
                ...
                ), f"Average response time too slow under load: {avg_response_time:.3f}s"
                assert (
                    requests_per_second > 10
                ), f"Throughput too low: {requests_per_second:.1f} req/s"

                print(
                    f"Concurrent performance: {requests_per_second:.1f} req/s, avg response: {avg_response_time:.3f}s"
                )
            else:
                pytest.skip(
                    "No successful concurrent requests to measure"
                )

        except Exception as e:
            pytest.skip(
                f"Concurrent request performance test skipped: {e}"
            )
    ...
    async def test_database_query_optimization(
        ...
        if not auth_headers:
            pytest.skip(
                "Authentication required for database performance test"
            )
        ...
        try:
            ...
            if query_times:
                ...
                assert (
                    max_query_time < 1.0
                ), f"Max query time too slow: {max_query_time:.3f}s"

                print(
                    f"Database query performance: avg={avg_query_time:.3f}s, max={max_query_time:.3f}s"
                )
            else:
    ...
    async def test_sustained_load_performance(
        ...
        if not auth_headers:
            pytest.skip(
                "Authentication required for sustained load test"
            )
        ...
        try:
            ...
            while time.time() < end_time:
                ...
                ):  # Don't count missing endpoints as errors
            ...
            if response_times:
                ...
                ), f"Average response time degraded under sustained load: {avg_response_time:.3f}s"
                assert (
                    p95_response_time < 2.0
                ), f"95th percentile response time too high: {p95_response_time:.3f}s"
                assert (
                    error_rate < 0.05
                ), f"Error rate too high under sustained load: {error_rate:.1%}"
                assert (
                    requests_per_second > 5
                ), f"Throughput too low under sustained load: {requests_per_second:.1f} req/s"

                print(
                    f"Sustained load results: {requests_per_second:.1f} req/s, avg={avg_response_time:.3f}s, p95={p95_response_time:.3f}s, errors={error_rate:.1%}"
                )
            else:
                pytest.skip(
                    "No successful requests during sustained load test"
                )
    ...
    async def test_response_time_consistency(self, client: AsyncClient):
        ...
        try:
            ...
            if response_times:
                ...
                assert (
                    stdev_time < mean_time
                ), f"Response times too inconsistent: mean={mean_time:.3f}s, stdev={stdev_time:.3f}s"
                assert (
                    max_time < mean_time * 5
                ), f"Max response time too far from mean: max={max_time:.3f}s, mean={mean_time:.3f}s"

                print(
                    f"Response time consistency: mean={mean_time:.3f}s, stdev={stdev_time:.3f}s, min={min_time:.3f}s, max={max_time:.3f}s"
                )
            else:
                pytest.skip(
                    "No successful health checks for consistency test"
                )
