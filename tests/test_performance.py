...
class TestCriticalPathPerformance:
    ...
    def test_authentication_performance(self, test_client):
        ...
        assert avg_time < 1.0, f"Average auth time {avg_time:.3f}s exceeds 1.0s threshold"
    ...
    def test_chat_message_performance(self, test_client):
        ...
        if create_response.status_code not in [200, 201]:
            pytest.skip("Cannot create conversation for performance test")
        ...
        assert avg_time < 2.0, f"Average message time {avg_time:.3f}s exceeds 2.0s threshold"
    ...
    def test_document_upload_performance(self, test_client):
        ...
        if response.status_code in [200, 201]:
            ...
            assert upload_time < 5.0, f"Upload time {upload_time:.3f}s exceeds 5.0s threshold"
    ...
    async def test_concurrent_request_performance(self, test_client):
        ...
        assert total_time < 10.0, f"Concurrent requests took {total_time:.3f}s, exceeds 10.0s threshold"
...
class TestDatabasePerformance:
    ...
    async def test_database_query_performance(self, mock_session):
        ...
        for i in range(10):
            await mock_session.execute("SELECT COUNT(*) FROM conversations")
            await mock_session.execute("SELECT * FROM conversations LIMIT 10")
        
        end_time = time.time()
        total_time = end_time - start_time
        
        # Database operations should be fast with mocking
        assert total_time < 1.0, f"DB operations took {total_time:.3f}s, exceeds 1.0s threshold"
    ...
    async def test_bulk_insert_performance(self, mock_session):
        ...
        records = [{"id": i, "content": f"Record {i}"} for i in range(1000)]
        ...
        assert bulk_time < 2.0, f"Bulk insert took {bulk_time:.3f}s, exceeds 2.0s threshold"
...
class TestMemoryUsage:
    ...
    def test_memory_usage_during_processing(self):
        ...
        assert memory_increase < 100, f"Memory increased by {memory_increase:.1f}MB, exceeds 100MB threshold"
    ...
    def test_no_memory_leaks_in_loops(self):
        ...
        assert memory_diff < 10, f"Potential memory leak: {memory_diff:.1f}MB increase after cleanup"
...
class TestCachePerformance:
    ...
    def test_cache_hit_performance(self):
        ...
        assert lookup_time < 0.1, f"Cache lookups took {lookup_time:.3f}s, exceeds 0.1s threshold"
    ...
    def test_cache_miss_handling(self):
        ...
        assert miss_time < 1.0, f"Cache miss handling took {miss_time:.3f}s, exceeds 1.0s threshold"