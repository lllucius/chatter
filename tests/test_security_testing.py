...
class TestSecurityTesting:
    ...
    async def test_sql_injection_protection_validation(self, client: AsyncClient, auth_headers: Dict[str, str]):
        ...
        try:
            ...
            for payload in sql_payloads:
                ...
                if response.status_code != 404:  # If endpoint exists
                    ...
                    assert response.status_code != 200, f"Potential SQL injection vulnerability with payload: {payload}"
                    
                    # Check for SQL error messages in response
                    response_text = response.text.lower()
                    sql_errors = ["sql", "mysql", "postgresql", "sqlite", "syntax error", "database"]
                    for error_term in sql_errors:
                        assert error_term not in response_text, f"SQL error exposed with payload: {payload}"
                ...
                if auth_headers:
                    ...
                    if search_response.status_code not in [404, 400]:  # If endpoint exists and accepts query
                        search_text = search_response.text.lower()
                        for error_term in sql_errors:
                            assert error_term not in search_text, f"SQL error exposed in search with payload: {payload}"
    ...
    async def test_xss_prevention(self, client: AsyncClient, auth_headers: Dict[str, str]):
        ...
        try:
            for payload in xss_payloads:
                ...
                reg_response = await client.post("/api/v1/auth/register", json=malicious_user)
                if reg_response.status_code in [200, 201]:
                    # If registration succeeds, the response should have escaped the payload
                    response_text = reg_response.text
                    assert "<script>" not in response_text, f"XSS vulnerability: unescaped script tag with payload: {payload}"
                    assert "javascript:" not in response_text, f"XSS vulnerability: javascript: protocol with payload: {payload}"
                    assert "onerror=" not in response_text, f"XSS vulnerability: event handler with payload: {payload}"
                ...
                if auth_headers:
                    ...
                    chat_response = await client.post("/api/v1/chats", json=malicious_chat, headers=auth_headers)
                    if chat_response.status_code in [200, 201]:
                        chat_data = chat_response.json()
                        chat_title = chat_data.get("title", "")
                        assert "<script>" not in chat_title, f"XSS vulnerability in chat title with payload: {payload}"
                        ...
                        if msg_response.status_code in [200, 201]:
                            ...
                            assert "<script>" not in message_content, f"XSS vulnerability in message with payload: {payload}"
    ...
    @pytest.mark.security
    async def test_authentication_security_brute_force_protection(self, client: AsyncClient):
        ...
        try:
            ...
            if later_attempts:
                ...
                rate_limited = any(attempt["status_code"] == 429 for attempt in later_attempts)
                if not rate_limited:
                    # Check for increasing response times (basic protection)
                    response_times = [attempt["response_time"] for attempt in failed_attempts if attempt["response_time"] > 0]
                    if len(response_times) >= 5:
                        ...
                        if later_avg <= early_avg:
                            print("Warning: No apparent brute force protection detected")
    ...
    @pytest.mark.security
    async def test_authorization_checks_and_token_validation(self, client: AsyncClient, auth_headers: Dict[str, str]):
        ...
        try:
            ...
            for endpoint in protected_endpoints:
                response = await client.get(endpoint, headers=malformed_headers)
                
                if response.status_code == 404:
                    continue  # Endpoint doesn't exist
                
                # Should reject malformed authorization header
                assert response.status_code == 401, f"Endpoint {endpoint} should reject malformed auth headers"
    ...
    @pytest.mark.security
    async def test_file_upload_security_malicious_detection(self, client: AsyncClient, auth_headers: Dict[str, str]):
        """Test file upload security including malicious file detection."""
        if not auth_headers:
            pytest.skip("Authentication required for file upload security test")
        ...
        try:
            ...
            for malicious_file in malicious_files:
                ...
                if len(malicious_file["content"]) > 5 * 1024 * 1024:  # > 5MB
                    assert response.status_code in [400, 413], f"Large file should be rejected: {malicious_file['filename']}"
                
                # Executable files should be rejected
                if malicious_file["filename"].endswith(".exe"):
                    assert response.status_code == 400, f"Executable file should be rejected: {malicious_file['filename']}"
                ...
                if "../" in malicious_file["filename"]:
                    ...
                    if response.status_code in [200, 201]:
                        ...
                        stored_filename = uploaded_data.get("filename", "")
                        assert "../" not in stored_filename, "Path traversal not prevented"
                ...
                if "\x00" in malicious_file["filename"]:
                    if response.status_code in [200, 201]:
                        uploaded_data = response.json()
                        stored_filename = uploaded_data.get("filename", "")
                        assert "\x00" not in stored_filename, "Null bytes not sanitized"
    ...
    @pytest.mark.security
    async def test_input_validation_and_sanitization(self, client: AsyncClient, auth_headers: Dict[str, str]):
        ...
        try:
            ...
            malicious_inputs = [
                ...
                {"type": "unicode", "value": "émojì 🚀 中文 русский العربية"},
                {"type": "control_chars", "value": "\x00\x01\x02\x03\x04\x05"},
                {"type": "html", "value": "<b>Bold</b> <i>Italic</i>"},
                {"type": "json_injection", "value": '{"malicious": true}'},
            ...
            for malicious_input in malicious_inputs:
                ...
                if response.status_code in [200, 201]:
                    ...
                    if malicious_input["type"] == "control_chars":
                        assert not any(ord(c) < 32 for c in stored_name if c != '\n' and c != '\t'), "Control characters not sanitized"
                    
                    # Extremely long strings should be truncated
                    if malicious_input["type"] == "long_string":
                        assert len(stored_name) < 1000, "Long string not truncated"
                ...
                if auth_headers:
                    ...
                    chat_response = await client.post("/api/v1/chats", json=test_chat, headers=auth_headers)
                    ...
                    if chat_response.status_code in [200, 201]:
                        ...
                        if malicious_input["type"] == "control_chars":
                            assert not any(ord(c) < 32 for c in stored_title if c != '\n' and c != '\t'), "Control characters not sanitized in chat title"
    ...
    @pytest.mark.security
    async def test_security_headers_validation(self, client: AsyncClient):
        """Test security headers validation."""
        try:
            response = await client.get("/healthz")
            
            if response.status_code != 200:
                pytest.skip("Health endpoint not available for header testing")
                ...
                if header not in headers:
                    ...
                    if isinstance(expected_value, list):
                        if headers[header] not in expected_value:
                            incorrect_headers.append(f"{header}: {headers[header]} (expected one of {expected_value})")
                    elif headers[header] != expected_value:
                        incorrect_headers.append(f"{header}: {headers[header]} (expected {expected_value})")
            ...
            if incorrect_headers:
                print(f"Incorrect security headers: {incorrect_headers}")
            
            if disclosed_info:
                print(f"Information disclosure headers: {disclosed_info}")
    ...
    @pytest.mark.security
    async def test_information_disclosure_prevention(self, client: AsyncClient):
        ...
        try:
            ...
            for endpoint in test_endpoints:
                ...
                sensitive_terms = [
                    ...
                    "stack trace", "traceback", "exception", "error:",
                    "debug", "development", "test", "admin"
                ]
                
                disclosed_terms = []
                for term in sensitive_terms:
                    if term in response_text and len(response_text) > 100:  # Avoid false positives on short responses
                        disclosed_terms.append(term)
                
                if disclosed_terms:
                    print(f"Potential information disclosure in {endpoint}: {disclosed_terms}")
                
                # Should not return detailed error messages for 404s
                if response.status_code == 404:
                    assert len(response.text) < 1000, f"404 response too detailed for {endpoint}"
    ...
    @pytest.mark.security
    async def test_cors_configuration_security(self, client: AsyncClient):
        ...
        try:
            ...
            for origin in test_origins:
                ...
                response = await client.options("/api/v1/chats", headers=headers)
                ...
                if "Access-Control-Allow-Origin" in cors_headers:
                    ...
                    if origin == "https://evil.com" and allowed_origin == origin:
                        print("Warning: CORS reflects arbitrary origins")
