...
class TestConfigurableSeederBasics:
    ...
    def test_yaml_config_loading_with_missing_file(self):
        ...
        seeder = ConfigurableSeeder(config_path="/nonexistent/path.yaml", session=None)
        assert seeder.config == {}  # Should handle missing file gracefully
...
class TestMethodImplementations:
    ...
    async def test_demo_embedding_spaces_method_exists(self):
        ...
        seeder = DatabaseSeeder(session=mock_session)
        result = await seeder._create_demo_embedding_spaces(skip_existing=True)
    ...
    async def test_test_conversations_method_exists(self):
        ...
        result = await seeder._create_test_conversations([], skip_existing=True)
        assert result == 0
    ...
    async def test_test_documents_method_exists(self):
        ...
        # Test with empty user list
        result = await seeder._create_test_documents([], skip_existing=True)
        assert result == 0
...
class TestErrorHandling:
    ...
    async def test_seeding_handles_exceptions(self):
        ...
        seeder = DatabaseSeeder(session=mock_session)
        seeder._count_users = AsyncMock(side_effect=Exception("Database error"))
        
        with pytest.raises(Exception):
            await seeder.seed_database(mode=SeedingMode.MINIMAL, force=False)
...
class TestSkipExistingLogic:
    ...
    async def test_skip_existing_when_users_exist(self):
        ...
        result = await seeder.seed_database(mode=SeedingMode.DEVELOPMENT, force=False)
    ...
    async def test_force_mode_bypasses_existing_check(self):
        ...
        seeder = DatabaseSeeder(session=mock_session)
        seeder._count_users = AsyncMock(return_value=5)  # Users exist
        seeder._seed_development_data = AsyncMock()  # Mock the seeding method
        
        result = await seeder.seed_database(mode=SeedingMode.DEVELOPMENT, force=True)
        
        # Should not skip seeding
        assert "skipped" not in result or "reason" not in result.get("skipped", {})
