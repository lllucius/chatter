...
class TestSemanticSearchDimensionFix:
    ...
    def test_hybrid_search_helper_dimension_choices(self):
        ...
        query_1536 = [0.1] * 1536
        column_1536 = HybridVectorSearchHelper.choose_search_column(
            query_1536, prefer_exact_match=True
        )
        assert column_1536 == 'embedding'

        # Test 3072-dimension query (should use raw_embedding for exact match)
        query_3072 = [0.1] * 3072
        column_3072 = HybridVectorSearchHelper.choose_search_column(
            query_3072, prefer_exact_match=True
        )
        assert column_3072 == 'raw_embedding'

        # Test 768-dimension query (should use raw_embedding for exact match)
        query_768 = [0.1] * 768
        column_768 = HybridVectorSearchHelper.choose_search_column(
            query_768, prefer_exact_match=True
        )
        assert column_768 == 'raw_embedding'

        # Test prefer_exact_match=False (should use computed_embedding for non-1536 queries)
        column_no_exact = HybridVectorSearchHelper.choose_search_column(
            query_3072, prefer_exact_match=False
        )
    ...
    def test_query_vector_preparation(self):
        ...
        prepared_raw_3072 = (
            HybridVectorSearchHelper.prepare_query_vector(
                query_3072, 'raw_embedding'
            )
        )
    ...
    def test_fallback_scenario_fix(self):
        ...
        query_embedding = [
            0.1
        ] * 3072  # 3072-dimension query like in the error
    ...
    def test_normalize_embedding_to_fixed_dim(self):
        ...
        truncated = normalize_embedding_to_fixed_dim(
            large_embedding, 1536
        )
        ...
        unchanged = normalize_embedding_to_fixed_dim(
            exact_embedding, 1536
        )
    ...
    def test_dimension_consistency_across_operations(self):
        ...
        for query_vector, description in test_cases:
            ...
            print(
                f"✓ {description}: {len(query_vector)} -> raw:{len(raw_prepared)}, computed:{len(computed_prepared)}"
            )
