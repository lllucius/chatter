...
class TestPGVectorSQLConcatenationFix:
    ...
    @pytest.mark.asyncio
    async def test_pgvector_initialization_prevents_sql_concatenation(
        self,
    ):
        ...
        with patch(
            'chatter.core.vector_store.PGVector'
        ) as mock_pgvector:
            ...
            assert (
                'create_extension' in call_args.kwargs
            ), "create_extension parameter should be present"
            assert (
                call_args.kwargs['create_extension'] is False
            ), "create_extension should be False to prevent SQL concatenation"

            # Verify other expected parameters
            assert (
                call_args.kwargs['async_mode'] is True
            ), "async_mode should be True"
            assert (
                call_args.kwargs['use_jsonb'] is True
            ), "use_jsonb should be True"
            assert (
                call_args.kwargs['collection_name'] == "test_collection"
            )

    @pytest.mark.asyncio
    async def test_pgvector_store_initialization_with_extension_disabled(
        self,
    ):
