...
class TestStreamingService:
    ...
    async def test_create_stream_auto_correlation_id(self, mock_get_correlation_id):
    ...
    async def test_stream_token(self):
        ...
        # Find token event
        token_events = [e for e in events if hasattr(e, 'event_type') and e.event_type == StreamingEventType.TOKEN]
    ...
    async def test_stream_tool_call_lifecycle(self):
        ...
        end_events = []
        async for chunk in self.service.stream_tool_call_end(stream_id, tool_name, tool_result):
    ...
    async def test_stream_activity_tracking(self):
        ...
        updated_activity = self.service.active_streams[stream_id]["last_activity"]
...
class TestStreamingServiceIntegration:
    ...
    async def test_complete_streaming_workflow(self):
        ...
        # Thinking event
        async for event in self.service.stream_thinking(stream_id, "Analyzing the question..."):
            events_collected.append(event)
        
        # Multiple tokens
        for token in ["Hello", " ", "world", "!"]:
            async for event in self.service.stream_token(stream_id, token):
                events_collected.append(event)
    ...
    async def test_error_recovery_workflow(self):
        ...
        async for _ in self.service.stream_token(stream_id, "Recovered"):
            pass
        
        # Complete successfully
        async for _ in self.service.stream_complete(stream_id, {"recovered": True}):
            pass
    ...
    async def test_stream_isolation(self):
        ...
        async for _ in self.service.stream_token(stream1_id, "stream1_content"):
            pass
        async for _ in self.service.stream_token(stream2_id, "stream2_content"):
            pass
        ...
        assert stream1_info["correlation_id"] != stream2_info["correlation_id"]
