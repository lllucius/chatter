...
import sys
sys.modules['chatter.core.workflow_security'] = unittest.mock.MagicMock()
sys.modules['chatter.core.workflow_security'].workflow_security_manager = mock_security_manager
...
class TestToolCallLimitFix:
    ...
    @pytest.mark.asyncio
    async def test_tool_call_limit_generates_proper_final_response(self):
    ...
    async def test_should_continue_routes_to_finalize_at_limit(self):
        ...
        state = {
            "messages": [HumanMessage(content="Hello"), message_with_tools],
            "tool_call_count": 3
        }
    ...
    async def test_workflow_completes_without_recursion_error(self):
        ...
        mock_llm = ToolLimitMockLLM(max_calls=100)  # Would try to make many calls
