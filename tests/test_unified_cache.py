...
from chatter.core.cache_interface import CacheInterface, CacheConfig, CacheStats
...
class TestCacheInterface:
    ...
    async def test_memory_cache_batch_operations(self, memory_cache):
        ...
        # Test mget
        result = await memory_cache.mget(["key1", "key2", "key3", "nonexistent"])
    ...
    async def test_memory_cache_key_generation(self, memory_cache):
        ...
        key2 = memory_cache.make_key("type", param1="value1", param2="value2")
        ...
        assert not memory_cache.is_valid_key("very_long_key" * 50)  # Too long
...
class TestCacheFactory:
    ...
    def test_create_different_cache_types(self, factory):
        ...
        model_cache = factory.create_cache(CacheType.MODEL_REGISTRY, CacheBackend.MEMORY)
        assert isinstance(model_cache, CacheInterface)
        assert model_cache.config.key_prefix == "model_registry"
        
        # Test workflow cache
        workflow_cache = factory.create_cache(CacheType.WORKFLOW, CacheBackend.MEMORY)
        assert isinstance(workflow_cache, CacheInterface)
        assert workflow_cache.config.key_prefix == "workflow"
        
        # Test tool cache
        tool_cache = factory.create_cache(CacheType.TOOL, CacheBackend.MEMORY)
    ...
    def test_convenience_methods(self, factory):
        ...
        model_cache = factory.create_model_registry_cache(backend=CacheBackend.MEMORY)
        assert isinstance(model_cache, CacheInterface)
        
        workflow_cache = factory.create_workflow_cache(backend=CacheBackend.MEMORY)
        assert isinstance(workflow_cache, CacheInterface)
        
        tool_cache = factory.create_tool_cache(backend=CacheBackend.MEMORY)
        assert isinstance(tool_cache, CacheInterface)
    ...
    async def test_health_check_all(self, factory):
        ...
        # Create some caches
        factory.create_cache(CacheType.MODEL_REGISTRY, CacheBackend.MEMORY)
    ...
    async def test_get_stats_all(self, factory):
        ...
        cache1 = factory.create_cache(CacheType.MODEL_REGISTRY, CacheBackend.MEMORY)
        cache2 = factory.create_cache(CacheType.WORKFLOW, CacheBackend.MEMORY)
...
class TestUnifiedModelRegistryCache:
    ...
    def mock_cache(self):
        ...
        cache.make_key = MagicMock(side_effect=lambda *args, **kwargs: ":".join(str(arg) for arg in args))
        return cache
    ...
    @pytest.mark.asyncio
    async def test_default_provider_operations(self, registry_cache, mock_cache):
        """Test default provider cache operations."""
        # Test set default provider
        mock_cache.set.return_value = True
        result = await registry_cache.set_default_provider(ModelType.CHAT, "provider1")
    ...
    @pytest.mark.asyncio
    async def test_provider_data_operations(self, registry_cache, mock_cache):
        """Test provider data cache operations."""
        provider_data = {"id": "provider1", "name": "Test Provider", "active": True}
        
        # Test set provider
        mock_cache.set.return_value = True
        result = await registry_cache.set_provider("provider1", provider_data)
        assert result is True
    ...
    async def test_list_operations(self, registry_cache, mock_cache):
        ...
        result = await registry_cache.set_provider_list(
            provider_type="openai", active_only=True, page=1, per_page=20,
            providers=providers, total=1
        )
        assert result is True
...
class TestUnifiedWorkflowCache:
    ...
    def mock_cache(self):
        """Create mock cache for testing."""
        cache = AsyncMock(spec=CacheInterface)
        cache.make_key = MagicMock(side_effect=lambda *args, **kwargs: f"unified:{':'.join(str(arg) for arg in args)}")
        return cache
    ...
    @pytest.mark.asyncio
    async def test_workflow_operations(self, workflow_cache, mock_cache):
        ...
        # Test put workflow
        mock_cache.set.return_value = True
        result = await workflow_cache.put("openai", "chat", config, workflow_obj)
        assert result is True
        mock_cache.set.assert_called_once()
        ...
        result = await workflow_cache.get("openai", "chat", {"different": "config"})
    ...
    async def test_workflow_stats(self, workflow_cache, mock_cache):
        ...
        mock_stats = CacheStats(
            total_entries=5, cache_hits=10, cache_misses=2,
            hit_rate=0.8, memory_usage=1024, evictions=1, errors=0
        )
    ...
    async def test_workflow_key_generation(self, workflow_cache):
        ...
        key1 = workflow_cache._generate_cache_key("openai", "chat", config1)
        key2 = workflow_cache._generate_cache_key("openai", "chat", config2)
        assert key1 == key2
        
        # Different configs should generate different keys
        config3 = {"param1": "different_value", "param2": "value2"}
        key3 = workflow_cache._generate_cache_key("openai", "chat", config3)
        assert key1 != key3
...
async def test_cache_integration():
    ...
    await workflow_cache.put("openai", "chat", config_data, workflow_obj)
