...
class TestEventRouter:
    ...
    def test_add_handlers(self, router, mock_handler):
        ...
        category_id = router.add_category_handler(EventCategory.SECURITY, mock_handler)
        ...
        assert mock_handler in router._category_handlers[EventCategory.SECURITY]
        assert mock_handler in router._type_handlers["test.event"]
    
    @pytest.mark.asyncio
    async def test_route_event(self, router, mock_emitter, mock_handler):
        """Test routing an event to handlers and emitters."""
        # Set up router
        router.register_emitter(EventCategory.REALTIME, mock_emitter)
        router.add_global_handler(mock_handler)
        router.add_category_handler(EventCategory.REALTIME, mock_handler)
...
class TestEventIntegration:
    ...
    async def test_full_event_flow(self):
        ...
        try:
            ...
            assert any("security.auth.login_failure" in et for et in event_types)
