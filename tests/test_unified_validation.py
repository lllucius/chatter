...
class TestValidationEngine:
    ...
    def test_validate_input_success(self):
        ...
        result = validation_engine.validate_input(
            "test@example.com", "email", DEFAULT_CONTEXT
        )
    ...
    def test_validate_input_failure(self):
        ...
        result = validation_engine.validate_input(
            "invalid-email", "email", DEFAULT_CONTEXT
        )
    ...
    def test_validate_security_failure(self):
        ...
        result = validation_engine.validate_security(
            "<script>alert('xss')</script>", DEFAULT_CONTEXT
        )
        assert not result.is_valid
        assert any(
            isinstance(error, SecurityValidationError)
...
class TestInputValidator:
    ...
    def test_email_validation(self):
        ...
        result = validator.validate(
            "test@example.com", "email", context
        )
        assert result.is_valid
...
class TestSecurityValidator:
    ...
    def test_xss_detection(self):
        ...
        # Should detect XSS
        result = validator.validate(
            "<script>alert('xss')</script>", "xss_check", context
        )
        assert not result.is_valid
    ...
    def test_sql_injection_detection(self):
        ...
        result = validator.validate(
            "'; DROP TABLE users; --", "sql_injection_check", context
        )
        assert not result.is_valid

        # Normal text should pass
        result = validator.validate(
            "normal query text", "sql_injection_check", context
        )
        assert result.is_valid
    ...
    def test_path_traversal_detection(self):
        ...
        result = validator.validate(
            "../../../etc/passwd", "path_traversal_check", context
        )
        assert not result.is_valid

        # Normal path should pass
        result = validator.validate(
            "normal/path/file.txt", "path_traversal_check", context
        )
        assert result.is_valid
...
class TestBusinessValidator:
    ...
    def test_model_consistency(self):
        ...
        data = {"model_type": "EMBEDDING", "dimensions": None}
        result = validator.validate(data, "model_consistency", context)
        assert not result.is_valid
...
class TestValidationContext:
    ...
    def test_context_overrides(self):
        ...
        context = ValidationContext(max_length_overrides={"email": 100})
...
class TestValidationResult:
    ...
    def test_result_merging(self):
        ...
        result2 = ValidationResult(
            is_valid=False, errors=[ValidationError("Error")]
        )
