...
class TestUniversalTemplateStreamingFix:
    ...
    def test_universal_template_variable_nodes_creation(self):
        ...
        empty_node = WorkflowNodeFactory.create_node('variable', 'empty_test', {})
    ...
    def test_no_old_validation_errors_thrown(self):
        ...
        try:
            node = WorkflowNodeFactory.create_node('variable', 'test_node', {})
            errors = node.validate_config()
            
            # Ensure no errors mention required variable_name
            for error in errors:
                assert "variable_name" not in error.lower() or "required" not in error.lower(), \
                    f"Found old validation error: {error}"
                    
        except ValueError as e:
            error_msg = str(e).lower()
            assert not ("variable_name" in error_msg and ("required" in error_msg or "defined" in error_msg)), \
    ...
    def test_variable_node_backward_compatibility_maintained(self):
        ...
        node1 = WorkflowNodeFactory.create_node(
            'variable',
            'test1', 
            {'variable_name': 'my_explicit_var', 'operation': 'set', 'value': 'test'}
        )
        ...
        node2 = WorkflowNodeFactory.create_node(
            'variable',
            'test2',
            {'variableName': 'myLegacyVar', 'operation': 'set', 'value': 'test'}
        )
    ...
    def test_multiple_variable_nodes_in_workflow(self):
        ...
        node_configs = [
            ('set_capabilities', {'operation': 'set', 'value': {'enable_memory': True}}),
        ...
        for node_id, config in node_configs:
            node = WorkflowNodeFactory.create_node('variable', node_id, config)
            assert node.variable_name.startswith('var_') or config.get('variable_name')
        ...
        assert len(set(var_names)) == len(var_names), "Variable names should be unique"