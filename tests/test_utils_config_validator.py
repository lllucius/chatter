...
class TestConfigurationValidator:
    ...
    def test_validate_secret_key_valid(self):
        ...
        # Arrange
        valid_key = "this_is_a_very_strong_secret_key_with_sufficient_length"
    ...
    def test_validate_secret_key_weak_default(self):
        ...
        for weak_key in weak_keys:
            with pytest.raises(ValueError, match="Secret key is weak or using default value"):
    ...
    def test_validate_secret_key_weak_pattern(self):
        ...
        for weak_key in weak_pattern_keys:
            with pytest.raises(ValueError, match="Secret key is weak or contains insecure pattern"):
                self.validator.validate_secret_key(weak_key)
    ...
    def test_validate_secret_key_repeated_characters(self):
        ...
        # Act & Assert
        with pytest.raises(ValueError, match="Secret key contains only repeated characters"):
    ...
    def test_validate_secret_key_simple_numeric(self):
        ...
        for numeric_key in numeric_keys:
            with pytest.raises(ValueError, match="Secret key is a simple numeric sequence"):
    ...
    def test_validate_database_settings_missing_url(self, mock_settings):
    ...
    @patch('chatter.utils.config_validator.settings')
    def test_validate_database_settings_invalid_url(self, mock_settings):
        ...
        # Assert
        assert any("DATABASE_URL format is invalid" in error for error in self.validator.errors)

    @patch('chatter.utils.config_validator.settings')
    def test_validate_database_settings_weak_credentials(self, mock_settings):
        """Test database validation with weak credentials."""
        # Arrange
        mock_settings.database_url = "postgresql://chatter:chatter_password@localhost/db"
        ...
        # Assert
        assert any("weak credentials" in error for error in self.validator.errors)

    @patch('chatter.utils.config_validator.settings')
    def test_validate_database_settings_invalid_pool_size(self, mock_settings):
        ...
        # Assert
        pool_errors = [e for e in self.validator.errors if "pool" in e.lower()]
        assert len(pool_errors) >= 2

    @patch('chatter.utils.config_validator.settings')
    def test_validate_security_settings_weak_secret(self, mock_settings):
        ...
        # Assert
        assert any("SECRET_KEY is too weak" in error for error in self.validator.errors)

    @patch('chatter.utils.config_validator.settings')
    def test_validate_security_settings_invalid_token_expiry(self, mock_settings):
        """Test security validation with invalid token expiry."""
        # Arrange
        mock_settings.secret_key = "very_strong_secret_key_that_is_long_enough"
        ...
        # Assert
        token_errors = [e for e in self.validator.errors if "token" in e.lower()]
    ...
    def test_validate_all_success(self, mock_settings):
        ...
        mock_settings.database_url = "postgresql://user:strongpass@localhost/db"
        mock_settings.db_pool_size = 5
        mock_settings.db_max_overflow = 10
        ...
        with patch.object(self.validator, '_validate_performance_settings'), \
    ...
    def test_report_validation_results_with_errors_and_warnings(self, mock_logger):
    ...
    def test_report_validation_results_success(self, mock_logger):
        ...
        # Assert
        mock_logger.info.assert_called_with("Configuration validation completed successfully")
...
class TestConfigurationValidatorIntegration:
    ...
    def test_url_parsing_integration(self):
        ...
        for url, should_be_valid in url_tests:
            ...
            with patch('chatter.utils.config_validator.settings') as mock_settings:
                ...
                # Assert
                if should_be_valid:
                    url_errors = [e for e in validator.errors if "format is invalid" in e]
                    assert len(url_errors) == 0, f"URL {url} should be valid but got errors: {validator.errors}"
                else:
                    url_errors = [e for e in validator.errors if "format is invalid" in e or "parsing failed" in e]
                    assert len(url_errors) > 0, f"URL {url} should be invalid but no format errors found"
    ...
    def test_secret_key_strength_levels(self):
        ...
        key_tests = [
            ...
            ("this_is_a_very_strong_random_secret_key_with_good_entropy", True),
        ...
        for key, should_pass in key_tests:
            ...
            if should_pass:
                ...
                try:
                    ...
                    pytest.fail(f"Key '{key}' should be valid but failed with: {e}")
            else:
...
class TestConfigurationValidatorEdgeCases:
    ...
    def test_validate_secret_key_unicode(self):
        ...
        # Arrange
        unicode_key = "强密钥_with_unicode_characters_and_sufficient_length"
    ...
    def test_database_url_parsing_exception(self, mock_settings):
        ...
        with patch('chatter.utils.config_validator.urlparse', side_effect=Exception("Parse error")):
            # Act
            self.validator._validate_database_settings()
            
            # Assert
            assert any("parsing failed" in error for error in self.validator.errors)
    ...
    def test_validator_state_persistence(self):
        ...
        # Assert
        assert len(self.validator.errors) >= 2  # Original + database error
