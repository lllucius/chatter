...
from chatter.core.validation import DEFAULT_CONTEXT, ValidationEngine
...
class TestValidationEngineValidateMethod:
    ...
    def test_validate_method_exists(self):
        ...
        assert hasattr(
            engine, 'validate'
        ), "ValidationEngine should have a validate method"
    ...
    def test_validate_with_invalid_validator(self):
        ...
        assert "Validator 'nonexistent' not found" in str(
            exc_info.value
        )
    ...
    def test_validate_missing_required_fields(self):
        ...
        assert (
            len(result.errors) == 2
        )  # Should have errors for missing nodes and edges
        error_messages = [str(error) for error in result.errors]
        assert any(
            "Missing required field: nodes" in msg
            for msg in error_messages
        )
        assert any(
            "Missing required field: edges" in msg
            for msg in error_messages
        )
