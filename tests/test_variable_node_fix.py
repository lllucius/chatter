...
class TestVariableNodeFix:
    ...
    def test_variable_node_with_explicit_variable_name(self):
        ...
        node = VariableNode(
            'test_node', {'variable_name': 'my_variable'}
        )
    ...
    def test_variable_node_factory_creation(self):
        ...
        node = WorkflowNodeFactory.create_node(
            node_type='variable', node_id='test_variable', config={}
        )
    ...
    def test_variable_node_backward_compatibility(self):
        ...
        node1 = VariableNode(
            'test_node1', {'variable_name': 'snake_case_var'}
        )
        assert node1.variable_name == 'snake_case_var'

        # Test camelCase (legacy support)
        node2 = VariableNode(
            'test_node2', {'variableName': 'camelCaseVar'}
        )
    ...
    def test_variable_node_warning_logged_for_auto_generated_names(
        self,
    ):
    ...
    async def test_variable_node_execution(self):
        ...
        # Test set operation
        node = VariableNode(
            'test_node', {'operation': 'set', 'value': 'test_value'}
        )
