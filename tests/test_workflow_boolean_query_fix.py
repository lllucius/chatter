...
def test_workflow_boolean_query_consistency():
    ...
    assert 'WorkflowDefinition.is_public == True' in content, (
    ...
    for i, line in enumerate(lines):
        if 'WorkflowDefinition.is_public' in line and '== True' not in line:
            # Allow certain contexts like assignments
            if ('=' in line and '==' not in line) or ('is_public=' in line):
                continue
            # If we find a bare WorkflowDefinition.is_public in a query context, that's bad
            if any(keyword in line for keyword in ['where(', 'or_(', 'and_(']):
                pytest.fail(
    ...
    assert 'WorkflowTemplate.is_public == True' in content, (
        "Template queries should also use explicit 'is_public == True' check"
    )
    
    assert 'WorkflowTemplate.is_builtin == True' in content, (
...
def test_workflow_access_logic_scenarios():
    ...
    scenarios = [
        ...
        ("user1", "user2", True, True, "User accesses other's public workflow"), 
        ("user1", "user2", False, False, "User cannot access other's private workflow"),
        ("user1", "user2", None, False, "User cannot access workflow with NULL is_public"),
    ]
