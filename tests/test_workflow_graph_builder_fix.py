...
class TestWorkflowGraphBuilderArgumentFix:
    ...
    def test_create_llm_node_wrapper_no_duplicate_args(self):
        ...
        try:
            ...
            if "got multiple values for argument 'llm'" in str(e):
                raise AssertionError(f"Argument duplication not fixed: {e}")
            else:
                # Other TypeError might be expected due to fake LLM
                print(f"ℹ️  Expected TypeError (not argument duplication): {e}")
        except Exception as e:
            # Other exceptions might be expected due to fake LLM
            print(f"ℹ️  Expected exception (not argument duplication): {e}")
    ...
    def test_kwargs_filtering_preserves_other_params(self):
        ...
        assert "llm" not in captured_kwargs, "llm should be filtered out of kwargs"
        assert "tools" not in captured_kwargs, "tools should be filtered out of kwargs"
        assert captured_kwargs.get("extra_param1") == "value1", "extra_param1 should be preserved"
        assert captured_kwargs.get("extra_param2") == "value2", "extra_param2 should be preserved"
