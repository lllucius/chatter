...
class TestWorkflowLimitManager:
    ...
    def test_concurrent_limit_check(self):
        ...
        self.limit_manager.check_concurrent_limit(self.user_id, self.limits)
        
        # Add workflows up to limit
        for i in range(self.limits.max_concurrent):
            self.limit_manager.user_workflow_counts[self.user_id] = i + 1
    ...
    def test_workflow_tracking_lifecycle(self):
        ...
        assert self.limit_manager.user_workflow_counts[self.user_id] == 1
        ...
        assert self.workflow_id not in self.limit_manager.active_workflows
        assert self.limit_manager.user_workflow_counts[self.user_id] == 0
    ...
    def test_resource_limit_checking(self):
        ...
        with pytest.raises(WorkflowResourceLimitError) as exc_info:
            self.limit_manager.check_workflow_limits(self.workflow_id, self.limits)
        assert exc_info.value.limit_type == "max_tokens"
        
        # Reset for memory test
        self.limit_manager.active_workflows[self.workflow_id].tokens_used = 0
        
        # Test memory limit
        self.limit_manager.update_workflow_usage(
            self.workflow_id, memory_delta_mb=self.limits.max_memory_mb + 1
        )
    ...
    async def test_timeout_context_manager(self):
        ...
        with pytest.raises(WorkflowTimeoutError) as exc_info:
            async with self.limit_manager.workflow_timeout_context(
                self.workflow_id, timeout_seconds=0.1, timeout_type="test"
            ):
    ...
    async def test_enhanced_token_streaming_all_workflows(self):
        ...
        with patch('chatter.core.dependencies.get_workflow_manager') as mock_get_manager:
            ...
            for workflow_type in ["rag", "tools", "full"]:
                ...
                token_chunks = [c for c in chunks if c.type == "token"]
                assert len(token_chunks) > 0, f"No token chunks for {workflow_type}"
