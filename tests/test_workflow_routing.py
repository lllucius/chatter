...
def test_workflow_router_no_duplicate_prefix():
    ...
    with open('chatter/api/workflows.py', 'r') as f:
        content = f.read()
...
def test_workflow_router_inclusion_in_main():
    ...
    # Read the main file
    with open('chatter/main.py', 'r') as f:
        content = f.read()
    ...
    assert re.search(workflows_inclusion_pattern, content, re.MULTILINE), (
...
def test_no_double_workflows_in_routes():
    ...
    for route in expected_routes:
        ...
        assert '/workflows/workflows/' not in route, (
