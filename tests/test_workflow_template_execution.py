...
def test_workflow_generation_methods():
    ...
    nodes, edges = service._generate_plain_workflow(mock_template, merged_input)
    ...
    assert llm_config['temperature'] == 0.9, f"Expected 0.9, got {llm_config['temperature']}"
    assert llm_config['max_tokens'] == 500, f"Expected 500, got {llm_config['max_tokens']}"
    
    # Template defaults should be preserved when not overridden
    assert llm_config['model'] == "gpt-4", f"Expected gpt-4, got {llm_config['model']}"
    ...
    nodes, edges = service._generate_rag_workflow(mock_template, merged_input)
    
    assert len(nodes) == 4, f"Expected 4 nodes for RAG workflow, got {len(nodes)}"
    assert len(edges) == 3, f"Expected 3 edges for RAG workflow, got {len(edges)}"
    
    node_ids = [node['id'] for node in nodes]
    assert 'retrieval' in node_ids, "RAG workflow should have retrieval node"
    
    print("✅ RAG workflow generation test passed")
    
    # Test tools workflow generation
    mock_template.workflow_type = "tools"
    nodes, edges = service._generate_tools_workflow(mock_template, merged_input)
    
    assert len(nodes) == 3, f"Expected 3 nodes for tools workflow, got {len(nodes)}"
    ...
    nodes, edges = service._generate_full_workflow(mock_template, merged_input)
    
    assert len(nodes) == 4, f"Expected 4 nodes for full workflow, got {len(nodes)}"
    node_ids = [node['id'] for node in nodes]
    assert 'retrieval' in node_ids, "Full workflow should have retrieval"
