...
class TestWorkflowValidationFixes:
    ...
    def test_new_node_types_validation(self):
        ...
        definition_data = {
            ...
            "nodes": [
                ...
                {"id": "memory-1", "type": "memory", "position": {"x": 150, "y": 200}, "data": {"label": "Memory"}},
                {"id": "retrieval-1", "type": "retrieval", "position": {"x": 250, "y": 200}, "data": {"label": "Retrieval"}},
                {"id": "model-1", "type": "model", "position": {"x": 350, "y": 200}, "data": {"label": "Model"}},
                {"id": "end-1", "type": "end", "position": {"x": 450, "y": 200}, "data": {"label": "End"}}
            ],
            "edges": [
                {"id": "e1", "source": "start-1", "target": "memory-1", "type": "custom", "data": {}},
                {"id": "e2", "source": "memory-1", "target": "retrieval-1", "type": "custom", "data": {}},
                {"id": "e3", "source": "retrieval-1", "target": "model-1", "type": "custom", "data": {}},
                {"id": "e4", "source": "model-1", "target": "end-1", "type": "custom", "data": {}}
            ],
            "metadata": {}
        }
        ...
        # Should be valid
        assert result.is_valid, f"Validation should pass. Errors: {[str(e) for e in result.errors]}"
    ...
    def test_comprehensive_workflow_from_error_log(self):
        ...
        definition_data = {
            ...
            "description": "asdf", 
            "nodes": [
                {"id":"start-1","type":"start","position":{"x":50,"y":200},"data":{"label":"Start","nodeType":"start","config":{"isEntryPoint":True}}},
                {"id":"memory-1","type":"memory","position":{"x":250,"y":200},"data":{"label":"Memory Manager","nodeType":"memory","config":{"enabled":True,"window":20}}},
                {"id":"retrieval-1","type":"retrieval","position":{"x":450,"y":200},"data":{"label":"Document Retrieval","nodeType":"retrieval","config":{"collection":"knowledge_base","topK":5}}},
                {"id":"model-1","type":"model","position":{"x":650,"y":200},"data":{"label":"Chat Model","nodeType":"model","config":{"temperature":0.7,"maxTokens":1000,"systemMessage":"Use the retrieved context to answer questions."}}},
                {"id":"conditional-1","type":"conditional","position":{"x":850,"y":200},"data":{"label":"Tool Decision","nodeType":"conditional","config":{"condition":"should_use_tools","branches":{"true":"tool-1","false":"end"}}}},
                {"id":"tool-1","type":"tool","position":{"x":1050,"y":100},"data":{"label":"Tool Execution","nodeType":"tool","config":{"tools":["web_search","calculator"],"parallel":False}}}
            ],
            "edges": [
                {"id":"e1","source":"start-1","target":"memory-1","type":"custom","data":{}},
                {"id":"e2","source":"memory-1","target":"retrieval-1","type":"custom","data":{}},
                {"id":"e3","source":"retrieval-1","target":"model-1","type":"custom","data":{}},
                {"id":"e4","source":"model-1","target":"conditional-1","type":"custom","data":{}},
                {"id":"e5","source":"conditional-1","target":"tool-1","type":"custom","data":{"label":"needs tools","condition":"has_tool_calls"},"sourceHandle":"true"},
                {"id":"e6","source":"tool-1","target":"model-1","type":"custom","data":{"label":"tool results"}}
            ],
            "metadata": {
                "name":"asdf",
                "description":"asdf",
        ...
        assert result.is_valid, f"Validation should pass after fixes. Errors: {[str(e) for e in result.errors]}"
    ...
    def test_dictionary_validation_still_works(self):
        ...
        result = validator.validate(definition_data_with_non_dict_nodes, "workflow_definition", context)
        
        # Should be invalid
        assert not result.is_valid, "Validation should fail for non-dictionary nodes"
        
        # Should have specific error about node being a dictionary
        node_dict_errors = [e for e in result.errors if "Node 0 must be a dictionary" in str(e)]
        assert len(node_dict_errors) == 1, f"Should have error about node being dictionary: {[str(e) for e in result.errors]}"
    ...
    def test_edge_dictionary_validation(self):
        ...
        result = validator.validate(definition_data_with_non_dict_edges, "workflow_definition", context)
        
        # Should be invalid
        assert not result.is_valid, "Validation should fail for non-dictionary edges"
        
        # Should have specific error about edge being a dictionary
        edge_dict_errors = [e for e in result.errors if "Edge 0 must be a dictionary" in str(e)]
        assert len(edge_dict_errors) == 1, f"Should have error about edge being dictionary: {[str(e) for e in result.errors]}"
